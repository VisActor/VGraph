...
vGraph provides React node components to satisfy the requirements of displaying complex nodes in graph scenarios, allowing users to easily customize nodes with React. Please read the usage documentation carefully before use.

## Key Configurations

- `Graph`: `dag` layout (`rankDir: TB`), default node width `280`, height `80`, expandable to `150`.
- `Viewer`: Implements node content and interaction using a combination of `Tabs + Tag + Icon` in `setNode`.
- `hideDetails`: Simplifies the display when zoomed out to ensure performance and readability of large graphs.
...
function App() {
  ...
  function setNode(node) {
    ...
    return React.createElement(
      ...
      { style: boxStyle },
      resizeIcon,
      React.createElement(Tag, { checkable: true, color: 'arcoblue', defaultChecked: true, style: tagStyle }, 'Normal'),
      node.get('name'),
      content,
      collapseIcon
    );
  ...
  if (!graph) return React.createElement('div', null, 'Loading...');
