...
Data description: The file architecture of the Flare toolkit, where a radial tree diagram is recommended for display when the number of child nodes expands rapidly.
Interaction: `panZoom canvas`: view navigation; `hover node`: highlight start and end paths; `drag node`: drag node; `click node`: collapse/expand the tree structure.
