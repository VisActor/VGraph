...
category: examples
group: practiceCases
title: Compact Tree Expand/Collapse
cover: https://lf9-dp-fe-cms-tos.byteorg.com/obj/bit-cloud/VGraph/site-exampleCovers/dp_compactbox.gif
link: practiceCases/CompactBox
option:
---
# Compact Tree Expand/Collapse

Compact tree scenario: collapsed nodes show a child count badge, and clicking the badge expands the subtree.
...
var graph = new TreeGraph({
  ...
  setDefaultNode: function(nodeData) {
    ...
    return { label: nodeData.id, type: 'category', radius: 4, color: NODE_COLORS[Math.round(Math.random()*9)], width: 190, height: 40,
      anchors: [[0,0.5],[1,0.5]],
      icons: nodeData.children ? [{ type: 'category', show: 'hover', position: [1,0.5], offsets: [20,0],
        setStyles: function() { return { fillStyle: '#3073F2', icon: '&#xe617;', cursor: 'pointer' }; },
...
function showSum(nodeData, e) {
  ...
  if (!sumLayer) {
    sumLayer = CountBadgeUtils.init(node, {
      position: 'right', text: String(count), color: node.get('color'),
