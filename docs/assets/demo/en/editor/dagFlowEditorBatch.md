...
category: examples
group: editor
title: Pipeline Editing - Batch Add Nodes
cover:
...
function batchAdd(editor, id) {
  // graph is the Graph object instance, you can also directly manipulate it to add nodes
  const graph = editor.getGraph();
  // Data snapshot before the operation, used for undo
  const formerData = editor.getSnapshot();
  console.log(formerData, editor);
  const branch = graph.add('node', { name: 'Condition Node' });
  const branchId = branch.get('id');
  // Connect the operation node and the new node
  graph.add('edge', { source: id, target: branchId });
  const condition1 = graph.add('node', { name: 'Condition Branch 1' });
  const condition2 = graph.add('node', { name: 'Condition Branch 2' });
  // Connect the condition node and branch nodes
  graph.add('edge', {
    source: branchId,
  ...
  // Re-layout, keeping the relative position of the operated node
  editor.reLayout(id);
  // Select the newly added branch node and ensure it's in the viewport
...
function initEditor() {
  ...
  const editor = new DAGFlowEditor({
    ...
    onClickNode(target, event) {
      ...
      if (relatedTarget && relatedTarget.type === 'icon') {
