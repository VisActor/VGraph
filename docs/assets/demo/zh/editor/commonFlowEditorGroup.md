...
- `CommonFlowEditor`：编辑器实例，负责增删改查与操作栈。
- `brushSelect`：左键框选节点/边/分组并同步选中态。
- `stack.execute('add'|'remove')`：执行分组与取消分组。
  ...
  useEffect(() => {
    ...
    canvas.ondrop = (e) => {
      ...
      _editor.addNode({
        x: point.x,
        y: point.y,
        name: e.dataTransfer.getData('name') || `新增节点${g.getNodes().length}`,
    ...
    { name: '重做', handler: () => editor.redo() },
    {
      ...
      handler: () => {
        ...
        editor.stack?.execute('add', {
          type: 'group',
          configs: {
            name: `新增分组${editor.getGraph().getGroups().length + 1}`,
            children: node.concat(group)
          }
        });
      }
    },
    { name: '取消组合', handler: () => editor.stack?.execute('remove', { ungroup: true }) },
