...
- `Graph`：layout 为 dag（rankDir TB），setDefaultNode 宽 450、高 360，setDefaultEdge 为 vLine。
- `Viewer`：setNode 返回大卡片（TitleCard + 两个 StatusCard），StatusCard 内 Statistic、Progress；hideDetails。
