...
A Path is a graphic object used to create a path graphic instance, containing information such as the path's drawing commands and parameters, for the rendering engine to draw and interact with.

## Example

```typescript
import { Path } from '@visactor/vgraph';
...
<p><img src="/vgraph/guide/shape/path.png" width="300"></p>

## Path Configuration

| Field | Type | Description |
| --- | --- | --- |
| path | (string \| number)[][] | A two-dimensional array, where each array represents a path command and its parameters. Currently supports M, L, H, V, A, C, Q, Z commands and their relative forms. Refer to [SVG Paths](https://developer.mozilla.org/en-US/docs/Web/SVG/Tutorial/Paths). |
...
> Note: Currently, only M, L, H, V, A, Z commands are supported. Curve commands such as C, S, Q, T are not yet supported.
