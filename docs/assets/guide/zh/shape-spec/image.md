...
图像 Image 是图形对象的一种，用于创建一个图像的图形实例，包含图像的位置、大小及图像纹理信息，用于渲染引擎进行绘制及交互操作。
...
| width  | number           | 图像宽度     |
| height | number           | 图像高度     |
| url    | string \| object | 图像源路径   |

> 注：关于 left, top, width, height 配置可参考[Rect](/vgraph/guide/shape-spec/rect)
