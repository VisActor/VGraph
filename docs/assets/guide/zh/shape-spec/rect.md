...
矩形 Rect 是图形对象的一种，用于创建一个矩形的图形实例，包含矩形的位置、大小、圆角半径等信息，用于渲染引擎进行绘制及交互操作。
## 示例
...
<p><img src="/vgraph/guide/shape/rect.png" width="300"></p>
...
| width  | number             | 矩形宽度     |
| height | number             | 矩形高度     |
| radius | number \| number[] | 矩形圆角     |

> 注：矩形圆角，若指定为某数字`r`，则矩形四个角的圆角半径均为r；若指定为长度为4的数组`[r0, r1, r2, r3]`，则依次指定了矩形左上、右上、右下、左下的圆角半径；若指定为长度为2的数组`[r0, r1]`，则指定矩形左上、右上、右下、左下的圆角半径分别为 r0, r1, r1, r0。
