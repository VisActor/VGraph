...
路径 Path 是图形对象的一种，用于创建一条路径的图形实例，包含绘制路径的指令及参数信息，用于渲染引擎进行绘制及交互操作。
...
| 字段       | 类型      | 描述                                                                                                                            |
| ---------- | --------- | ------------------------------------------------------------------------------------------------------------------------------- |
| path       | (string \| number)\[\]\[\]   | 一个二维数组，每个数组表示一个路径指令及其参数。目前支持 M、L、H、V、A、C、Q、Z 指令及其相对形式。参考[SVG 路径](https://developer.mozilla.org/zh-CN/docs/Web/SVG/Tutorial/Paths) |
| startArrow | ArrowType | 起始箭头配置项                                                                                                                  |
| endArrow   | ArrowType | 终点箭头配置项                                                                                                                  |
| hitWidth   | number    | 事件触发的感应宽度                                                                                                              |

> 注：目前仅支持 M, L, H, V, A, Z 指令，暂不支持 C,S,Q,T 等曲线指令。
