import { dragCanvas, Node, Graph, panZoom, Group, RawTooltip, GroupUtils, NestedDAG } from '../../../src';
...
(() => {
  ...
  graph.on('canvas:click', () => {
    ...
    search([nodes[Math.round(Math.random() * 10)].get('id'), nodes[Math.round(Math.random() * 10)].get('id')]);
  });
