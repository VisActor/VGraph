import { TreeGraph, panZoom, dragCanvas, Node, Edge, GraphEvent } from '../../../src';
...
fetch('https://cdn-tos-cn.bytedance.net/obj/maat/img/cWl1eWlsaW4uZWxhaW5l/file_186bfc559e342.json')
...
graph.on('node:click', (e: GraphEvent) => {
  ...
  graph.getNodes().forEach((node: Node) => {
    ...
    if (node.get('rad') !== undefined && node.get('rad') !== Math.PI * 2 - label.rotate) {
