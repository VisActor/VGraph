...
(() => {
  ...
  const centerNodeIdx = nodes.findIndex((d: any) => d.id === 'c05b1b4b-2aa3-42fe-9df9-7ce7c8d7c232'); // 圆心节点的索引
...
function setBfsDepth(centerNodeIdx: number, graph: any) {
  ...
  bfs(
    ...
    (node: any, parent: any) => {
      if (parent) {
        node.set('depth', parent.get('depth') + 1 > depthUpperBound ? depthUpperBound : parent.get('depth') + 1);
      }
    },
    false
  );
