import { DAGFlowEditor, GraphEvent, Node, uuid, Graph, registerEdge, highlightRelations, attachableDragNode } from '../../src';
...
(() => {
  ...
  update.textContent = '更新子节点';
  div.appendChild(update);

  const error = document.createElement('button');
  error.textContent = '展示节点错误';
  ...
  graph.addBehavior(attachableDragNode, {
    ...
    findClosestNode(target: Node) {
      ...
      graph.getNodes().forEach((node: Node) => {
        ...
        if (id !== target.get('id') && id !== 'xgraphDagFlowRoot' && node.get('y') <= y) {
          const dist = Math.sqrt((x - node.get('x')) * (x - node.get('x')) +
  ...
  function batchChange(target: Node) {
    ...
    for (let i = rank + 1; i < maxRank; i++) {
      ...
      editor.graph.add('edge', { source: sourceNode.get('id'), target: newNode.get('id') });
      sourceNode = newNode;
    }
    editor.graph.add('edge', { source: sourceNode.get('id'), target: maxRankNode.get('id') });
  ...
  add.onclick = () => {
    const parent = editor.getSelectedNode();
    if (!parent) {
      return;
    }
    // 建议在表单生成一个 uuid，用于节点的 id，这样后续表单再编辑的更新会更方便
    const mockData = [
      { id: uuid(8), name: '分支1: 用户账号特征' },
      { id: uuid(8), name: '分支2: 用户账号特征' },
      { id: uuid(8), name: '兜底分支：以上未命中' }
    ];
    const formerData = editor.getSnapshot();
    // 先做了一个删除第一层的，如果有递归场景
    if (parent.targets.length !== 0) {
      for (let i = parent.targets.length - 1; i >= 0; i--) {
        const id = parent.targets[i];
        removeNode(graph.getNodeById(id));
      }
    }
    mockData.forEach((data) => {
      const newNode = graph.add('node', data);
      graph.add('edge', {
        source: parent.get('id'),
        target: newNode.get('id')
      });
    });
    editor.reLayout(parent.get('id'));
    const currentData = editor.getSnapshot();
    editor.batchChange({ formerData, currentData });
  };
  ...
  remove.onclick = () => {
    ...
    node && editor.removeNode(node.get('id'), true);
  }
  ...
  update.onclick = () => {
    ...
    const mockData = [
      { id: targets[0], name: '分支1: 新-用户账号特征' }, // 更新
      { id: targets[2], name: '分支2: 新-用户账号特征' }, // 删除 targets[1]
      {id: uuid(8), name: '分支3： 新-用户账号特征'}  // 新增
    ];
    const formerData = editor.getSnapshot();
    const ids:string[] = [];
    mockData.forEach((data: any) => {
      const child = graph.getNodeById(data.id);
      if (child) {
        graph.update(child, data);
      } else {
        const newNode = graph.add('node', data);
        graph.add('edge', {
          source: parent.get('id'),
    ...
    parent.targets.forEach((id: string) => {
      if (!ids.includes(id)) {
        removeNode(graph.getNodeById(id));
      }
    });
