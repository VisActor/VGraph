import { brushSelect, CommonFlowEditor, Node, Icon, Edge, getDefaultBizData } from '../../src';
...
(() => {
  ...
  const editor = new CommonFlowEditor({
    ...
    data: {
      nodes: [
        { id: 'Task1', x: 320, y: 80, type: 'rect', width: 140, height: 40, groupId: 'group1' },
        { id: 'Task2', x: 450, y: 160, type: 'rect', width: 140, height: 40, groupId: 'group1' },
        {
          x: 320,
          y: 310,
          name: 'Task3',
    ...
    setDefaultGroup(groupData) {
      return {
        ...
        titleSize: 72,
        title: {
          text: { text: groupData.id, fillStyle: '#626978', y: 100 },
          background: {
            fillStyle: '#F0F3F6',
          },
        },
  ...
  graph.addBehavior(brushSelect, {
    ...
    onSelect(item: Node | Edge) {
      if (item.type === 'edge') {
        if ((item.source as Node).states.includes('select') && (item.target as Node).states.includes('select')) {
          item.toFront();
