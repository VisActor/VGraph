...
function createDummyGroupNodes(data: any) {
  ...
  const groups = [] as any[];
  const parents = Array.from(new Set(nodes.map((d: any) => d.parent))).filter((d: any) => d !== '') as string[];
  ...
  for (const node of nodes) {
    ...
    } else if (!node.isGroup || node.nodeType?.includes('Cluster') || node.nodeType === 'CNSet') {
      newNodes.push(node);
    }
  }
  ...
  const graph = new Graph({
    ...
    setDefaultNode(nodeData) {
      ...
      return {
        ...
        strokeStyle: nodeData.children_count ? color[group >= 0 ? group % 13 : 13] : '#fff',
...
function autoForce(
  ...
  const strengthXY = multi || 0.05 / avgDegree / leaf > 0.5 ? 0.5 : 0.05 / avgDegree / leaf;
  ...
  const forces = {
    ...
    manybody: new ForceManyBody({
      options: {
        strength: (d: any) => (d.children_count ? Math.sqrt(d.children_count) * -distance : -distance),
      },
    }), // 力导向排斥力，整体依旧呈现力导向布局。
