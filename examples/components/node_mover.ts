...
import { panZoom, highlightRelations, brushSelect } from '../../src/behaviors';
import { fastrand } from '../../src/layouts/utils';
import { Node, Edge } from '../../src/models/entities';
import { Layer, Rect } from '../../src/renderer';
import { DAGLayout } from '../../src/layouts/dag';
import { Grid, Router, NodeMover, Stack, AddCommand, SelectCommand, MoveNodeCommand } from '../../src/components';
...
const edges = largeDecisionData.edges.filter((edge) => hasNode[edge.source] && hasNode[edge.target]);
const dataDecision = { nodes, edges };

// 构造新的graph
const rootNodeIds = nodes.filter((node) => [0, 2, 3].includes(node?.nodeType)).map((node) => node.id);
const rootNodeMap = {};
...
(() => {
  ...
  const graph = new Graph({
    ...
    setNodeStateStyles(state: string, data: any) {
      ...
      if (state === 'select') {
        return { lineWidth: 2, strokeStyle: '#3370FF' };
      }
      if (state === 'hover') {
  ...
  graph.addBehavior(brushSelect, {
    ...
    onChange(selected: any[]) {
      stack.execute('select', { selections: selected });
