const setReleaseVersion = require("./set-release-version");
...
function run() {
  ...
  const nextVersion = parsedNextVersion
    ? `${parsedNextVersion.major}.${parsedNextVersion.minor}.${parsedNextVersion.patch}`
    : null;

  console.log("[apply prerelease version]: ", preReleaseName, nextBump, nextVersion);

  setReleaseVersion(nextBump, preReleaseName, nextVersion, buildName);
}
