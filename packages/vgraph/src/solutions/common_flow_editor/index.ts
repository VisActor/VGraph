...
export type CommonFlowEditorOptions = {
  ...
  shortcuts?: {
    ...
    customShortcuts?:
      ...
      | ((
          defaultShortcuts: { [type: string]: HandlerOption },
          stack?: Stack
        ) => {
          [type: string]: HandlerOption;
        });
...
export class CommonFlowEditor {
  ...
  private initGraph() {
    ...
    if (
      options.scroller &&
      (options.scroller.enable === undefined ||
        options.scroller.enable === true)
    ) {
      let scrollerOptions = {
  ...
  private initComponents() {
    ...
    const { mode, nodeMover, edgeEditor, background, shortcuts, grid } =
      this.options;
    ...
    if (grid!.enable !== false) {
      ...
      const gridComponent = new Grid(this.graph, {
        ...grid!.options,
        step: gridStep,
        extraWidth: 0.5 * gridStep,
      }); // extraWidth 设置为一个 0.5 step, 防止擦边连线
    ...
    if (nodeMover?.enable !== false) {
      const nodeMoverOptions = Object.assign(
        defaultOptions.nodeMover.options,
        nodeMover?.options || {}
      );
    ...
    if (edgeEditor?.enable !== false) {
      const edgeEditorOptions = Object.assign(
        defaultOptions.edgeEditor.options,
        edgeEditor?.options || {}
      );
    ...
    if (shortcuts?.enable !== false) {
      let shortcutsHandlers = this.getDefaultShortcuts() as {
        [type: string]: HandlerOption;
      };
      if (shortcuts!.customShortcuts) {
        if ("function" === typeof shortcuts?.customShortcuts) {
          shortcutsHandlers = shortcuts?.customShortcuts(
            shortcutsHandlers,
            this.stack
          );
        } else {
          shortcutsHandlers = shortcuts?.customShortcuts as {
            [type: string]: HandlerOption;
          };
        }
      }
  ...
  private getDefaultOptions() {
    return {
      ...
      shortcuts: {
        enable: true,
        customShortcuts: (shortcuts: Record<string, HandlerOption>) =>
          shortcuts,
      },
      grid: {
        enable: true,
  ...
  private getDefaultShortcuts() {
    ...
    shortcuts.paste = {
      ...
      handler: (event: KeyboardEvent | ClipboardEvent) => {
        this.stack.execute("paste", {
          event,
          position: this.getLastMousePosition(),
        });
      },
    };
    return shortcuts;
  ...
  getLastMousePosition() {
    const point = this.graph.clientToCanvas(
      this.lastMouseClient[0],
      this.lastMouseClient[1]
    );
   ...
   * 返回组件实例。
   */
  getComponent<T extends keyof CommonFlowEditor["components"]>(
    name: T
  ): CommonFlowEditor["components"][T] {
   ...
   */
  setComponent<
    T extends keyof CommonFlowEditor["components"],
    Component extends CommonFlowEditor["components"][T]
  >(name: T, component: Component) {
