...
export function selectionIntoView(graph: Graph, autoScale = false) {
  ...
  ["group", "node"].forEach((type: string) => {
    selections?.[type]?.forEach((id: string) => {
      const entity =
        type === "group" ? graph.getGroupById(id) : graph.getNodeById(id);
  ...
  if (
    (width > containerBBox.width && height > containerBBox.height) ||
    !autoScale
  ) {
    ...
    if (maxY > height) {
      scale = Math.min(
        scale,
        (height - padding[2] - center.y) / (maxY - center.y)
      );
    }
    if (minX < 0) {
    ...
    if (maxX > width) {
      scale = Math.min(
        scale,
        (width - padding[1] - center.x) / (maxX - center.x)
      );
    }
