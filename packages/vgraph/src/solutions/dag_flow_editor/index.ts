...
const ROOT_ID = "vgraphDagFlowRoot";
...
export class DAGFlowEditor {
  ...
  private initEmptyEditor() {
    ...
    if (rankDir === "TB") {
      this.graph.translate(
        graphSize[0] / 2,
        padding![0] + node.get("height") / 2
      );
    } else {
      this.graph.translate(
        padding![3] + node.get("width") / 2,
        graphSize[1] / 2
      );
    }
  }
  ...
  moveNode(
    nodeId: string,
    parentId: string,
    pos: "siblingBefore" | "siblingAfter" | "source" | "target"
  ) {
    const graph = this.graph;
      ...
      switch (stackData.name) {
        ...
        default:
          if (stackData.name !== "update") {
            this.reLayout();
            stackData.data.redoMatrix &&
              graph.setMatrix(stackData.data.redoMatrix);
          }
          break;
      }
    }
  ...
  exportData(
    ...
    edgeFn?: (entity: Edge) => Record<string, any>
  ) {
    const graph = this.graph;
