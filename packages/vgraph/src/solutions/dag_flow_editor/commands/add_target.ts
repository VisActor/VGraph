...
export const AddTargetCommand = Object.assign({}, CommandBase, {
  ...
  getSnapshot(graph: Graph, args: AddTargetArgs) {
    ...
    const addedEdgesConfigs = [
      { id: relativeEdgeId, source: relativeId, target: id },
    ];
    ...
    originSources.sort((a: Edge, b: Edge) => {
      return targetIndexMap[a.get("target")] - targetIndexMap[b.get("target")];
    });
    ...
    return {
      ...
      removedEdges:
        args.linkChildren !== false
          ? originSources.map((edge: Edge) => edge.configs)
          : [],
