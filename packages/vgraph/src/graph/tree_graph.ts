...
export class TreeGraph extends GraphBase {
  ...
  data(data: TreeData) {
    ...
    this.layout(data);
    data.children &&
      this.addChildren(data.children, root, root.get("collapsed"), false);
    this.root = root;
    this.treeData = data;
    this.refresh();
  ...
  updateChildren(children: TreeData[] | undefined, parent: Node, hide = false) {
    ...
    if (children && childrenNodes.length === 0) {
      this.addChildren(
        children,
        parent,
        hide ? hide : parent.get("collapsed"),
        false
      );
      this.autoDraw();
      return;
  ...
  removeChild(child: Node, parent: Node | string, destroy = true) {
    ...
    const dataIndex = siblingData.findIndex(
      (n: any) => n.id === child.get("id")
    );
    if (dataIndex >= 0) {
  ...
  moveNode(node: Node, parent: Node, parentIndex?: number) {
    ...
    if (originParent) {
      ...
      const edge = node.edges.find(
        (e) => e.get("source") === originParent.get("id")
      );
      if (edge) {
        edge.destroy();
      }
    ...
    if (
      ...
      parentIndex < 0 ||
      parentIndex > parent.get("children").length
    ) {
   ...
   */
  getTreeData(
    fn?: (entity: Node | Edge) => Record<string, unknown>,
    data?: Node
  ): TreeData {
    if (!fn) {
      fn = (entity: any) => {
