...
export class GraphBase extends EventEmitter implements IGraph {
  ...
  private bindEvents() {
    ...
    [
      GRAPH_EVENTS.DRAW_START,
      GRAPH_EVENTS.DRAW_END,
      GRAPH_EVENTS.ANIMATION_START,
      GRAPH_EVENTS.ANIMATION_END,
  ...
  fitView() {
    ...
    const center = this.getViewCenter();
    container.translate(
      center.x - bbox.left - bbox.width / 2,
      center.y - bbox.top - bbox.height / 2
    );
  ...
  alignView(
    align: "lt" | "ct" | "rt" | "lc" | "cc" | "rc" | "lb" | "cb" | "rb" = "lt"
  ) {
    const container = this.container;
    const padding = this.getViewPadding();
   ...
   */
  filter(
    fn: (entity: Node | Edge | Group) => boolean,
    target: "node" | "edge" | "group"
  ) {
    this.set("emitGraphEvents", false);
    (
      Object.values(this.entityMap[target]) as Node[] | Edge[] | Group[]
    )?.forEach((entity: Node | Edge | Group) => {
      if (fn(entity)) {
        entity.show();
      } else {
        entity.hide();
   ...
   * 根据配置执行画布动画
   * @param {AnimateConfigs} configs - vgraph has well packed animates can be used by padding type and also general animates.
  ...
  clear() {
    ...
    Object.values(this.entityMap).forEach(
      (entities: Record<string, Node | Edge | Group>) => {
