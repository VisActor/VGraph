...
export class Edge extends Entity implements IEdge {
  ...
  constructor(
    ...
    const { source, target } = this.getRealLinkEntities(
      configs.source,
      configs.target
    );
    this.source = source;
  ...
  private getEdgeConfigs() {
    ...
    if (controlPoints) {
      // 合并起始点与终止点，并去除相邻重复的控制点
      let points = controlPoints.concat() ?? [];
      points.unshift(startPoint);
      points.push(endPoint);
      points = points.filter((item: number[], idx: number) => {
        // 去除相邻重复的点
        if (idx === 0) {
          return true;
        }
        return !(
          item[0] === points[idx - 1][0] && item[1] === points[idx - 1][1]
        ); // 判断是否与上一个点重复
      });
  ...
  updatePosition() {
    ...
    if (shapeMethods.updatePath) {
      shapeMethods.updatePath(
        this.layer,
        shapeMethods.getConfigsForShape(edgeConfigs)
      );
    } else {
      this.layer.clear();
  ...
  private getRealLinkEntities(sourceId?: string, targetId?: string) {
    ...
    for (let i = 0; i < targetAncestors.length; i++) {
      ...
      // 在同一个分组中
      if (index >= 0) {
        // index = 0, i = 0 情况下说明是嵌套关系，应该取它本身。
        return {
          source: this.getEntityById(
            sourceAncestors[index - 1 < 0 ? 0 : index - 1]
          ),
