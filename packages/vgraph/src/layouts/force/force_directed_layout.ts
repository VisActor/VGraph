...
export class ForceDirectedLayout extends LayoutBase {
  ...
  layout() {
    ...
    if (this.options.async) {
      shouldForceStart
        ? this.restart(this.options.alpha, this.options.maxIteration)
        : this.restart();
    } else {
  ...
  private configData(data: Graph | GraphStructure | ValidData) {
    if (
      ...
      this.nodes = (data as Graph)
        .getNodes()
        .map((node: Node) => node.configs as NodeData);
      this.edges = (data as Graph)
        .getEdges()
        .map((edge: Edge) => edge.configs as EdgeData);
    } else {
  ...
  private step() {
    ...
    if (this.nowIteration >= this.maxIteration) {
      ...
      this.graph instanceof Graph &&
        this.graph.emitEvent(GRAPH_EVENTS.LAYOUT_END);
    }
  }
  ...
  private tick() {
    for (let i = 0; i < this.options.tickIterations; i++) {
      ...
      this.alpha +=
        (this.options.alphaTarget - this.alpha) * this.options.alphaDecay;
      this.nowIteration += 1;
  ...
  initializeNodes(init?: string) {
    ...
    const nodeSize =
      this.options.nodeSize ?? (this.getMaxNodeSize() || DEFAULT_NODE_SIZE);
    if (init === "spiral") {
  ...
  configForces(forces?: { [key: string]: ForceBase } | Map<string, ForceBase>) {
    if (this.options.autoFDP) {
      ...
      const { forces, zoomRatio } = autoFDP(
        { nodes: this.nodes, edges: this.edges },
        {
          graphSize: [
            graph.get?.("width") ?? 800,
            graph.get?.("height") ?? 600,
          ],
          nodeSize,
        }
      );
