...
export class ForceCollision extends ForceBase {
  ...
  initialize(nodes: NodeData[]): void {
    ...
    if (this.options.radius) {
      this.radii = setNodesOption(this.nodes, this.options.radius);
  ...
  run(): void {
    ...
    if (this.nodes) {
      this.nodes.forEach((node) => {
        ...
        this.ri =
          this.radii.get(this.nowNode.id) ||
          0.5 *
            Math.max(
              this.widths.get(this.nowNode.id) || 0,
              this.heights.get(this.nowNode.id) || 0
            );
  ...
  accumulate(
    ...
    if (quad.data) {
      ...
      quad.r =
        ...
        0.5 *
          Math.max(
            this.widths.get(quad.data.id) || 0,
            this.heights.get(quad.data.id) || 0
          );
      return;
    }
