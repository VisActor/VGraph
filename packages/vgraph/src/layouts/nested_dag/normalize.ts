    ...
    if (!group.collapsed) {
      ...
      for (const edge of group.uniqueEdges) {
        if (edge.controlPoints) {
          edge.controlPoints = moveControlPoints(
            edge.controlPoints,
            offsetX - left,
            offsetY - top
          );
...
export function getBBoxForParent(
  ...
  // const node = parent;
  padding = normalizePadding((parent.padding as number) ?? padding ?? 20) as
    | number
    | number[];
  ...
  parent.height =
    maxY - minY + padding[0] + padding[2] + (parent.titleHeight || 0);
}
