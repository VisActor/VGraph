...
export function getNestedGraph(graph: Graph | GraphStructure, group: any) {
  ...
  for (let i = group.edges.length - 1; i >= 0; i--) {
    ...
    if (source === groupId) {
      ...
      if (targetNode.type === "group" || targetNode.belong) {
        configs.target = `_mockGroup_${
          targetNode.belong?.get("id") ?? targetNode.get("id")
        }`;
      ...
      if (sourceNode.type === "group" || sourceNode.belong) {
        configs.source = `_mockGroup_${
          sourceNode.belong?.get("id") ?? sourceNode.get("id")
        }`;
  ...
  for (let j = group.children.length - 1; j >= 0; j--) {
    ...
    for (let i = node.edges.length - 1; i >= 0; i--) {
      ...
      if (
        (source === id && childIds.includes(target)) ||
        (target === id && childIds.includes(source))
      ) {
        edges.push(configs);
      } else {
        // 外部连线改连到虚拟节点上
...
export function getBBoxForNode(graph: GraphStructure) {
  ...
  node.set(
    "height",
    maxY - minY + padding[0] + padding[2] + (node.get("titleHeight") || 0)
  );
}

export function recoverFromNestedGraph(
  graph: Graph | GraphStructure,
  nestedGraph: GraphStructure
) {
  ...
  nestedGraph.getEdges().forEach((edge: EdgeStructure) => {
    if (edge.configs.controlPoints) {
      edge.configs.controlPoints = moveControlPoints(
        edge.configs.controlPoints,
        offsetX,
        offsetY
      );
    }
...
export function assignControlPoints(
  graph: Graph | GraphStructure,
  configs: any,
  gapYs: any,
  vertical: boolean
...
function compressControlPoints(controlPoints?: number[][]) {
  ...
  if (!controlPoints || controlPoints.length === 0) {
    ...
    for (let i = 1; i < controlPoints.length; i++) {
      if (
        controlPoints[i][0] !==
          resControlPoints[resControlPoints.length - 1][0] ||
