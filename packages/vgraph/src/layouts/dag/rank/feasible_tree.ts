...
export function feasibleTree(graph: Graph | GraphStructure) {
  ...
  const start =
    nodes.find((node: Node | NodeStructure) => node.get("root")) ?? nodes[0];
  ...
  while (tightTree(tree, graph, nodeEdges) < size) {
    ...
    const delta =
      tree.nodes[edge.get("source")] !== undefined
        ? getSlack(graph, edge)
        : -getSlack(graph, edge);
...
function tightTree(
  ...
  nodeEdges: Record<string, (Edge | EdgeStructure)[]>
) {
