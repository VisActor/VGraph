...
export function horizontalCoordAssignment(
  ...
  ["U", "D"].forEach((vDirection: string) => {
    adjustLayers =
      vDirection === "U" ? copyArray(layers) : copyArray(layers).reverse();
      ...
      if (neighbors.length > 0) {
        ...
        for (let i = Math.floor(median), il = Math.ceil(median); i <= il; ++i) {
          ...
          if (
            ...
            !hasConflict(conflicts, nodeId, neighborId)
          ) {
...
export function horizontalCompaction(
  ...
  const { block, edgeMap } = getGraphByBlocks(
    data,
    layers,
    aligns.root,
    reverse,
  ...
  function firstWalk(nodeId: string) {
    xMap[nodeId] = edgeMap[nodeId].inEdges.reduce(
      (acc: number, edge: Edge | EdgeStructure) => {
  ...
  function secondWalk(nodeId: string) {
    const min = edgeMap[nodeId].outEdges.reduce(
      (acc: number, edge: Edge | EdgeStructure) => {
...
export function findSmallestWidthAlignment(
  ...
  xMaps: Record<string, Record<string, number>>
) {
...
export function alignCoords(
  ...
  ["U", "D"].forEach((vDirection: string) => {
    ["L", "R"].forEach((hDirecition: string) => {
      ...
      const delta =
        hDirecition === "L" ? min - extremes.min : max - extremes.max;
      if (delta) {
        Object.keys(xMap).forEach((k: string) => {
          xMap[k] += delta;
        });
...
function getGraphByBlocks(
  ...
  const edgeMap: Record<
    string,
    { inEdges: (Edge | EdgeStructure)[]; outEdges: (Edge | EdgeStructure)[] }
  > = {};
    ...
    row.forEach((node: Node | NodeStructure) => {
      ...
      if (prevNode) {
        ...
        const edge = subGraph.add("edge", {
          ...
          target: root,
          len: Math.max(
            getNodeEdgeSep(options, node, prevNode, reversed),
            prevMax
          ),
        });
...
// 获取 inner segment
function getInnerSegmentNode(
  nodeMap: { [key: string]: Node | NodeStructure },
