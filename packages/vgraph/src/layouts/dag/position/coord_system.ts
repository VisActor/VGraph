...
export const coordSystem = {
  ...
  alignPeerNodes(
    ...
    for (const layer of layers) {
      alignNodes(
        (layer as Node[]).filter(
          (node: Node | NodeStructure) => !node.get("dummy")
        ),
        align ?? "center"
      );
    }
...
function alignNodes(
  ...
  const minX = Math.min(
    ...nodes.map((node) => node.configs.x - node.configs.width / 2)
  );
  const maxX = Math.max(
    ...nodes.map((node) => node.configs.x + node.configs.width / 2)
  );
  const minY = Math.min(
    ...nodes.map((node) => node.configs.y - node.configs.height / 2)
  );
  const maxY = Math.max(
    ...nodes.map((node) => node.configs.y + node.configs.height / 2)
  );
