...
import {
  getBBoxForNode,
  getNestedGraph,
  recoverFromNestedGraph,
  nestingGraph,
} from "./preprocess/nested_graph";
...
export class DAGLayout extends LayoutBase {
  ...
  layout(param?: { clearRank: boolean }) {
    ...
    if (graph instanceof Graph) {
      ...
      if (graph.getGroups().length && !this.options.ignoreGroup) {
        ...
        nestedGraphs.forEach((nestedGraph: GraphStructure) => {
          ...
          const gapYs = this.layoutSingleGraph(nestedGraph, true, {
            ranker: "custom",
          }); // 再计算order和position
    ...
    const gapYs = this.layoutSingleGraph(
      graph,
      clearRank,
      ranked ? { ranker: "custom" } : {}
    );
    ...
    if (graph instanceof Graph) {
      if (nestedGraphs.length) {
        nestedGraphs.forEach((nestedGraph: GraphStructure) => {
          ...
          const { offsetX, offsetY } = recoverFromNestedGraph(
            graph,
            nestedGraph
          );
  ...
  private layoutSingleGraph(
    ...
    const rankers = Object.keys(bestOrders).sort(
    ...
    const align = this.validateAlignOption(
      this.options.rankDir,
      this.options.align
    );
  ...
  private validateAlignOption(
    rankDir: string,
    align: DAGLayoutOptions["align"]
  ) {
    if (align && ["L", "R", "T", "B"].includes(align)) {
      if (rankDir === "LR" || rankDir === "RL") {
        if (align === "L" || align === "R") {
          console.warn(
            'While "rankDir" is LR or RL, "align" can only be UL, UR, DL, DR, T or B, but got',
            align,
            "."
          );
      ...
      if (rankDir === "TB" || rankDir === "BT") {
        if (align === "T" || align === "B") {
          console.warn(
            'While "rankDir" is TB or BT, "align" can only be UL, UR, DL, DR, L or R, but got',
            align,
            "."
          );
          console.warn('"align" will be set to undefined.');
  ...
  private recoverRepeatEdges(graph: Graph | GraphStructure) {
    if (this.repeatEdges.length) {
      this.repeatEdges.forEach((edgeConfigs: any) => {
        ...
        if (controlPoints) {
          edgeConfigs.controlPoints = edgeConfigs.reversed
            ? controlPoints.toReversed()
            : controlPoints.concat();
        }
  ...
  private normalizeRank(graph: Graph | GraphStructure) {
    ...
    const ranks = Array.from(
      new Set(nodes.map((n: Node | NodeStructure) => n.get("rank")))
    );
    ...
    Object.keys(this.ranks).forEach((rank: string) => {
      ...
      entities.sort(
        (a: Node | NodeStructure, b: Node | NodeStructure) =>
          components[a.get("id")] - components[b.get("id")]
      );
    });
  }
  ...
  private adjustControlPoints(gapYs: Record<string, Record<string, number>>) {
    const { allControlPoints, adjustControlPoints, ignoreGroup, rankDir } =
      this.options;
    ...
    graph.getEdges().forEach((edge: Edge | EdgeStructure) => {
      ...
      // 对齐的节点连线是直线不需要计算
      if (
        (vertical && source.get("y") === target.get("y")) ||
        (!vertical && source.get("x") === target.get("x"))
      ) {
        return;
      }
      const sourceHeight = source.get(height);
      const targetHeight = target.get(height);
      const sourceGroup = source.get("groupId")
        ? graph.getGroupById(source.get("groupId"))
        : null;
      const targetGroup = target.get("groupId")
        ? graph.getGroupById(target.get("groupId"))
        : null;
      ...
      if (allControlPoints || sourceHeight < sourceRank.height) {
        ...
        controlPoints.unshift(
          vertical
            ? [
                !sourceGroup || sourceGroup.get("linkNode")
                  ? source.get("x")
                  : sourceGroup.get("x"),
                sourceRank.source,
              ]
      ...
      if (allControlPoints || targetHeight < targetRank.height) {
        controlPoints.push(
          vertical
            ? [
                !targetGroup || targetGroup.get("linkNode")
                  ? target.get("x")
                  : targetGroup.get("x"),
                targetRank.target,
              ]
      ...
      edge.set(
        "controlPoints",
        controlPoints.length > 0 ? controlPoints : undefined
      );
    });
  }
