...
export function getArcProjectionDist(
  target: Point,
  cx: number,
  cy: number,
  r: number,
  ...
  out[1] = vpoint[1];
  const d = Math.sqrt(
    (target.x - vpoint[0]) * (target.x - vpoint[0]) +
      (target.y - vpoint[1]) * (target.y - vpoint[1])
  );
  return d;
}
...
function getArcLength(params: any): number {
  ...
  const r =
    Math.PI * (rx + ry) * ((3 * lambda) / (10 + Math.sqrt(4 - 3 * lambda) + 1));
