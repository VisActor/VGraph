...
export function pathToSegments(
  ...
  for (const path of paths) {
    ...
    switch (path[0]) {
      ...
      case "m":
        lastPoint = relative
          ? toAbsolute(path[1], path[2], lastPoint)
          : {
              x: path[1],
              y: path[2],
            };
      ...
      case "a":
        ...
        const fs = path[5];
        point = relative
          ? toAbsolute(path[6], path[7], lastPoint)
          : {
              x: path[6],
              y: path[7],
            };
      ...
      case "c":
        ...
        const p2 = relative
          ...
          : { x: path[1], y: path[2] };
        const p3 = relative
          ? toAbsolute(path[3], path[4], lastPoint)
          : { x: path[3], y: path[4] };
        const p4 = relative
          ? toAbsolute(path[5], path[6], lastPoint)
          : { x: path[5], y: path[6] };
        currentLength = cubicUtil.getLength(
          [p1.x, p1.y],
          [p2.x, p2.y],
          [p3.x, p3.y],
          [p4.x, p4.y],
      ...
      case "q":
        ...
        if (path[5] !== undefined) {
          ...
          currentLength = quadratic.getLength(
            [qp1.x, qp1.y],
            [qp2.x, qp2.y],
            [qp3.x, qp3.y],
            24
...
export function getSegmentsBBox(segments: Segment[]): BBox {
  ...
  for (const segment of segments) {
    ...
    if (segment.type === "line") {
      ...
      currentBBox = cubicUtil.getCubicBox(
        [p0.x, p0.y],
        [p1.x, p1.y],
        [p2.x, p2.y],
        [p3.x, p3.y]
      ...
      currentBBox = quadratic.getQuadraticBBox(
        [p0.x, p0.y],
        [p1.x, p1.y],
        [p2.x, p2.y]
      );
...
export const ellipseUtil = {
  xAt(psi: number, rx: number, ry: number, cx: number, t: number) {
    return (
      rx * Math.cos(psi) * Math.cos(t) - ry * Math.sin(psi) * Math.sin(t) + cx
    );
  },
  yAt(psi: number, rx: number, ry: number, cy: number, t: number) {
    return (
      rx * Math.sin(psi) * Math.cos(t) + ry * Math.cos(psi) * Math.sin(t) + cy
    );
  },
