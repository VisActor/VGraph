...
export default class CanvasEvents {
  ...
  getShapeInLayer(point: Point, layer: LayerBase): ShapeBase | null {
    ...
    for (let i = children.length - 1; i >= 0; i--) {
      if (children[i].isLayer()) {
        ...
        this.isPointInShape(target, children[i], false, this.viewportPoint)
      ) {
        result = children[i];
      }
      if (result) {
    ...
    if (
      !result &&
      (layer.type === LAYER_TYPES.NODE || layer.type === LAYER_TYPES.GROUP)
    ) {
      if (layer.exactMatch) {
        return null;
      }
  ...
  isPointInShape(
    ...
    const target = shape.get("fixed")
      ? viewPoint
      : this.getActualPoint(point, shape);
  ...
  isPointInFill(point: Point, shape: ShapeBase) {
    ...
    switch (shape.type) {
      ...
      case SHAPE_TYPES.RHOMBUS:
        ({ left, top, width, height } = shape.configs);
        const ps = [
          [left + width / 2, top],
  ...
  isPointInStroke(point: Point, shape: ShapeBase) {
    ...
    switch (shape.type) {
      ...
      case SHAPE_TYPES.QUADRATIC:
        points = shape.get("points");
        return (
          quadraticUtil.getProjectionDist(
            [point.x, point.y],
            points[0],
            points[1],
            points[2]
