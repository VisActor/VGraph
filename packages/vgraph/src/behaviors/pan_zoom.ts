...
export const panZoom: any = {
  ...
  onTouchStart(e: ShapeEvent) {
    ...
    this.last = touchInfo;
    const x = touchInfo.reduce(
      (acc: number, touch: Touch) => touch.clientX + acc,
      0
    );
    const y = touchInfo.reduce(
      (acc: number, touch: Touch) => touch.clientY + acc,
      0
    );
    this.center = this.graph.canvas.clientToCanvas(
      x / touchInfo.length,
      y / touchInfo.length
    );
  },
  ...
  touchZoom(curr: ShapeEvent) {
    ...
    const dist =
      this.getDistance(curr[0], curr[1]) / this.getDistance(last[0], last[1]);
