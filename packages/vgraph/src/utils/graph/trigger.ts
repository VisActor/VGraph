...
export class Trigger {
  ...
  onClick(e: GraphEvent) {
    ...
    if (
      (trigger === "click" && evt?.button === 2) ||
      (trigger === "contextMenu" && evt?.button === 1)
    ) {
      return;
    }
  ...
  onMouseOver(e: GraphEvent) {
    ...
    if (
      !this.isTriggerShape(shape) ||
      (popupContainer && shape === this.curShape)
    ) {
      return;
    }
  ...
  show(e: GraphEvent) {
    ...
    if (entity.type === "edge" && !shape) {
      ...
      const size =
        keyShape.get("hitWidth") || keyShape.get("lineWidth") * 3 || 3;
      newStyles = {
        left: point.x,
        top: point.y,
        width: size,
