...
export class Snapline extends ComponentBase {
  ...
  constructor(graph: Graph, options?: ISnaplineOptions) {
    super(graph, options);
    const layer = new Layer({ id: "vgraphSnapline" });
    layer.capture = false;
  ...
  onMove(evt: {
    ...
    graph.getNodes().forEach((node: Node) => {
      ...
      if (abs(left + width / 2 - cx) < step) {
        ...
        !result.v &&
        (abs(left - minX) < step || abs(nmaxX - minX) < step)
      ) {
        ...
        !result.v &&
        !result.r &&
        (abs(left - maxX) < step || abs(nmaxX - maxX) < step)
      ) {
      ...
      if (abs(top + height / 2 - cy) < step) {
        ...
        !result.h &&
        (abs(top - minY) < step || abs(nmaxY - minY) < step)
      ) {
        ...
        !result.h &&
        !result.b &&
        (abs(top - maxY) < step || abs(nmaxY - maxY) < step)
      ) {
