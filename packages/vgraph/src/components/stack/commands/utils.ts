...
export function isEmptySelections(selections: {
  node: string[];
  edge: string[];
  group: string[];
}) {
  if (
    selections.node.length === 0 &&
    selections.edge.length === 0 &&
    selections.group.length === 0
  ) {
    return true;
  }
  return false;
...
export function getSelectionsSnapshot(
  ...
  function getGroupSnapshot(group: Group) {
    ...
    configs.group.push({
      ...group.configs,
      children: group.children.map((node: Node | Group) => node.get("id")),
    });
  }
...
export function getCleanSnapshot(
  ...
  function getCleanGroupSnapshot(group: Group) {
    ...
    groupEdges.forEach((edge: Edge) => {
      ...
      if (
        (maps.node[source] || maps.group[source]) &&
        (maps.node[target] || maps.group[target]) &&
        !maps.edge[id]
      ) {
        configs.edge.push(getEdgeConfigs(edge));
        maps.edge[id] = true;
...
export function batchAddRemote(
  graph: Graph,
  addConfigs: { node: any[]; edge: any[]; group: any[] },
  treeLike = false
) {
  ...
  if (treeLike) {
    ...
    const rootNodes = addConfigs.node.filter(
      ({ id: nodeId }: { id: string }) => {
        // 首先找到添加的节点中的根节点
        const node = graph.getNodeById(nodeId);
...
export function batchRemove(
  graph: Graph,
  removeConfigs: { node: any[]; edge: any[]; group: any[] }
) {
