...
export class Background extends ComponentBase {
  ...
  update() {
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="${total}" height="${total}">
        <defs>
          <pattern id="grid" width="${total}" height="${total}" patternUnits="userSpaceOnUse">
            <path d="M 0 ${first} L ${total} ${first} M ${first} 0 L ${first} ${total} M 0 ${second} L ${total} ${second} M ${second} 0 L ${second} ${total} M 0 ${third} L ${total} ${third} M ${third} 0 L ${third} ${total}" stroke="${gridStrokeColor}" opacity="${
        0.2 * gridOpacity
      }" fill="none" stroke-width="${0.1 * first}"/>
            <path d="M ${total} 0 L 0 0 0 ${total}" stroke="${gridStrokeColor}" fill="none" stroke-width="${
        0.1 * first
      } " opacity="${gridOpacity}"/>
          </pattern>
        </defs>
    ...
    `;
      ...
      <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="${total}" height="${total}">
        <defs>
          ...
          <rect width="${0.1 * first}" height="${0.1 * first}" rx="${
        0.1 * first
      }" ry="${0.1 * first}" fill="${dotFillColor}" opacity="${dotOpacity}"/>
          </pattern>
        </defs>
    ...
    this.container.style.backgroundImage = `url("data:image/svg+xml;base64,${btoa(
      imgString
    )}")`;
    this.refreshPosition();
  }
  ...
  initContainer() {
    ...
    div.classList.add("vgraph-background");
    parent.prepend(div);
    return div;
  }
