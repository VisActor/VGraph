...
export class Router {
  ...
  setDefaultOptions(options: RouterOptions) {
    this.options = Object.assign({}, this.options, options);
    this.finder =
      this.options.algorithm === "BStar"
        ? new BStarFinder(options)
        : new AStarFinder(options);
  }
  ...
  fallbackPath(controlPoints: number[][], edge: Edge) {
    ...
    if (length === 1) {
      ...
      if (
        startPoint[0] !== controlPoints[0][0] &&
        startPoint[1] !== controlPoints[0][1]
      ) {
      ...
      if (
        endPoint[0] !== controlPoints[length - 1][0] &&
        endPoint[0] !== controlPoints[length - 1][1]
      ) {
  ...
  findPath(
    ...
    if (this.grid) {
      ...
      for (const point of startPoints) {
        ...
        const startInfo = this.grid.directionWalkableWithMinDist(
          point[0],
          point[1],
          minDist
        ) as terminalInfo;
      ...
      for (const point of endPoints) {
        ...
        const endInfo = this.grid.directionWalkableWithMinDist(
          point[0],
          point[1],
          minDist
        ) as terminalInfo;
      ...
      if (controlPoints?.length) {
        const length = controlPoints.length;
        const [col, row] = this.grid?.getIndexByCoord(
          controlPoints[0][0],
          controlPoints[0][1]
        );
        const [endCol, endRow] = this.grid?.getIndexByCoord(
          controlPoints[length - 1][0],
          controlPoints[length - 1][1]
        );
      ...
      if (controlPoints && controlPoints.length > 2) {
        ...
        if (
          ...
          // 乘积 < 0, 说明出现折返情况。
          if (
            (startPoint[1] - controlPoints[1][1]) *
        ...
        if (
          ...
          // 水平方向
          if (
            (startPoint[0] - controlPoints[1][0]) *
              (controlPoints[0][0] - controlPoints[1][0]) <
            0
          ) {
            // 剔除起始点
