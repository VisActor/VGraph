...
export class ContinuousLegend extends ComponentBase {
  ...
  render() {
    ...
    // legend 图例元素
    const { track, rail, label, width, height, orient, color, labelStyles } =
      this.options;
    ...
    if (orient === "vertical") {
      ...
      minLabel = this.drawLabel(
        0,
        railHeight / 2 + spacing,
        "center",
        "top",
        minLabelConfigs
      );
      maxLabel = this.drawLabel(
        0,
        -railHeight / 2 - spacing,
        "center",
        "bottom",
  ...
  processData() {
    ...
    if (!color && !size) {
      ...
      if (
        typeof mappingValueMax === "number" &&
        typeof mappingValueMin === "number"
      ) {
    ...
    canvas.on("mousemove", (e: any) => {
      ...
      if (minLastPosition !== null) {
        ...
        if (minRectLength < 0) {
          ...
          minHandler.setMatrix(
            matrixMultiply([1, 0, 0, 1, length, length], translateMatrix)
          );
        ...
        if (totalLength >= length && translate[monitor] > 0) {
          ...
          maxHandler.setMatrix(
            matrixMultiply(
              [1, 0, 0, 1, -maxRectLength, -maxRectLength],
              translateMatrix
            )
          );
      ...
      // 水平布局的右边滑动条 垂直布局的下方滑动条
        ...
        if (totalLength >= length && translate[monitor] < 0) {
          ...
          minHandler.setMatrix(
            matrixMultiply(
              [1, 0, 0, 1, minRectLength, minRectLength],
              translateMatrix
            )
          );
          minRect.set(rectTransform, minRectLength);
      ...
      canvas.draw();
      if (slide.filter) {
        this.filterValue = this.scaleValue(
          leftRect.get(rectTransform),
          length - rightRect.get(rectTransform)
        );
  ...
  getScaleRange() {
    ...
    if (scale.type === "log") {
      this.valueDiffer =
        Math.pow(scaleBase, value.max) - Math.pow(scaleBase, value.min);
    } else if (scale.type === "pow") {
      this.valueDiffer =
        Math.pow(value.max, 1 / scaleBase) - Math.pow(value.min, 1 / scaleBase);
    } else {
    ...
    if (orient === "horizontal") {
      ...
      leftHandler = new Path({
        path: [
          ...
          [
            "A",
            handlerConfigs.size / 2,
            handlerConfigs.size / 2,
            0,
            0,
            0,
            leftX + handlerConfigs.size,
            bottomY,
          ],
          ["V", topY],
          [
