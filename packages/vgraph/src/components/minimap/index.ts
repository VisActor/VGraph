...
export class Minimap extends ComponentBase {
  ...
  updateDelegateShapes() {
    ...
    this.updateLayer(
      getDrawingOrderEntities(this.graph),
      stylerMap,
      (id: string, entity: Node | Group) => {
  ...
  setViewport(left: number, top: number, width: number, height: number) {
    ...
    let show = true;
    if (
      this.options.hideWindowWhenOverflow !== false &&
      left <= 0 &&
      top <= 0 &&
  ...
  onMouseLeave(e: MouseEvent) {
    this.options.enable = true;
    if (!this.dragging && e.type === "mouseup") {
      // 点击 viewport 区域，非拖拽。以鼠标为中心定位 viewport。
      this.clickToCenter(e);
    }
  ...
  getDefaultOptions() {
    return {
      ...
      getNodeStyles: (node: any) => ({
        fillStyle: node.get("fillStyle") || node.get("color") || "#fff",
      }),
      getGroupStyles: () => {
