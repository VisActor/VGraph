...
export class Grid extends ComponentBase {
  ...
  onUpdateStart(ev: GraphEvent) {
    if (
      !this.shouldIgnoreEntity(ev.target) &&
      !this.onMovingEntitiesMap[ev.target.get("id")]
    ) {
  ...
  onMoveStart(
    ev: GraphEvent & { batch?: boolean; targets: GraphEvent["target"][] }
  ) {
  ...
  refresh(alignGrid = false) {
    ...
    if (alignGrid) {
      ...
      for (const node of nodes) {
        ...
        node.configs.x =
          Math.round((x - left) / step - 0.5) * step + 0.5 * step + left;
        node.configs.y =
          Math.round((y - top) / step - 0.5) * step + 0.5 * step + top;
      }
    }
    for (const node of nodes) {
      const bbox = node.getBBox();
  ...
  shouldIgnoreGroup(group: Group) {
    return (
      !group.get("collapsed") || group.get("ignoreGrid") || !group.isVisible()
    );
  }
  ...
  addBBox(bbox: BBox) {
    ...
    let startCol = Math.floor(
      (left - this.gridData.left - this.options.extraWidth) / step
    );
    let endCol = Math.ceil(
      (left + width - this.gridData.left + this.options.extraWidth) / step
    );
    let startRow = Math.floor(
      (top - this.gridData.top - this.options.extraWidth) / step
    );
    let endRow = Math.ceil(
      (top + height - this.gridData.top + this.options.extraWidth) / step
    );
    if (
      startCol < BORDERS ||
      startRow < BORDERS ||
  ...
  expandedGrid(
    ...
    let newCols = cols;
    if (startCol < BORDERS) {
      const additionalCols = Math.max(
        Math.ceil(0.5 * cols),
        Math.ceil(-startCol) + BORDERS
      );
    ...
    if (endCol > cols - BORDERS) {
      newCols += Math.max(
        Math.ceil(0.5 * cols),
        Math.ceil(endCol - cols + BORDERS)
      );
    }
    if (startRow < BORDERS) {
      const additionalRows = Math.max(
        Math.ceil(0.5 * rows),
        Math.ceil(-startRow) + BORDERS
      );
  ...
  isWalkable(col: number, row: number) {
    if (
      col < 0 ||
      col >= this.gridData.cols ||
      row < 0 ||
      row >= this.gridData.rows
    ) {
      return false;
    }
    ...
    if (this.isWalkable(col, row) && this.isSuitableTerminal(pos[0], pos[1])) {
      ...
      if (conditions.length) {
        ...
        const distance = (a: { col: number; row: number }) =>
          (left + (a.col + 0.5) * step - x) ** 2 +
          (top + (a.row + 0.5) * step - y) ** 2;
  ...
  directionWalkableWithMinDist(x: number, y: number, minDist = 20) {
    ...
    for (let i = 0; i < 2; i++) {
      for (let j = 0; j < 2; j++) {
        ...
        if (
          this.isWalkable(useCol, useRow) &&
          this.isSuitableTerminal(x + xSign * minDist, y + ySign * minDist)
        ) {
          const condition = {
          ...
          certain =
            certain &&
            this.isWalkable(colWithExtra + xSign, rowWithExtra + ySign);
          certain =
            certain &&
            this.isWalkable(
              colWithExtra + xSign + [0, 1][i],
              rowWithExtra + ySign + [1, 0][i]
            );
          certain =
            certain &&
            this.isWalkable(
