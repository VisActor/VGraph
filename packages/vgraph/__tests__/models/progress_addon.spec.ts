...
describe("src/node_addons/progress", () => {
  ...
  it("init & update default progress should work", () => {
    ...
    expect(path.get("lineDash")).toEqual([10, 100]);
    ...
    expect(path.get("lineDash")).toEqual([50, 100]);
  });
  ...
  it("init circle progress should work", (done) => {
    ...
    expect(Math.round(circle.get("lineDash")[0])).toBe(
      Math.round(Math.PI * 60)
    );
    expect(Math.round(circle.get("lineDash")[1])).toBe(
