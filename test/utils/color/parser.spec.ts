import { colorParser, getRgbaValue, GradientColor, OrdinaryColor } from '../../../src/utils/color/parser';
describe('./utils/color.ts', () => {
  it('getRgbaValue should work', () => {
    ...
    expect(getRgbaValue('hsla(13, 100%, 11%, 0.4)')).toEqual([
      56, 12, 0, 0.4,
    ]);
  });
  ...
  it('OrdinaryColor should work', () => {
    ...
    expect(ordinaryColor.vec4()).toEqual([0.0784313725490196, 0.0784313725490196, 0.0784313725490196, 1]);
  ...
  it('GradientColor should word', () => {
    const gradientColor = colorParser('l(0) 0:#fff 0.5:#7ec2f3 1:#1890ff') as GradientColor;
    expect(gradientColor.toString()).toBe('l(0) 0:#ffffff 0.5:#7ec2f3 1:#1890ff ');
    ...
    expect(gradientColor.toString()).toBe('l(0) 0:#ffffee 0.5:#7ec2e2 1:#1890ee ');
    gradientColor.add('#000002');
    expect(gradientColor.toString()).toBe('l(0) 0:#fffff0 0.5:#7ec2e4 1:#1890f0 ');
    gradientColor.reverse();
    expect(gradientColor.toString()).toBe('l(0) 0:#00000f 0.5:#813d1b 1:#e76f0f ');
  });
