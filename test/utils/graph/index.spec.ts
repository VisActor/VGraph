...
describe('src/graph/index', () => {
  ...
  it('isPointInScreen should work', () => {
    expect(isPointInScreen({ x: -1, y: -1 }, { width: 800, height: 600 })).toBe(false);
    expect(isPointInScreen({ x: 0, y: -1 }, { width: 800, height: 600 })).toBe(false);
    expect(isPointInScreen({ x: 1000, y: 0 }, { width: 800, height: 600 })).toBe(false);
    expect(isPointInScreen({ x: 50, y: 50 }, { width: 800, height: 600 })).toBe(true);
  });
});