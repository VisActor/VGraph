...
describe('src/graph/tree_graph', () => {
  ...
  it('data should work', () => {
    ...
    if (graph.root) {
      ...
      const children = graph.root.get('children').map((child: any) => graph.getNodeById(child.id));
  ...
  it('get & setZoomRatio should work', () => {
    ...
    expect(graph.getMatrix()).toEqual([1, 0, 0, 1, 133.33333333333337, 133.33333333333334]);
  });
  ...
  it('bugfix: add existed node should work', () => {
    ...
    graph.updateChildren([
      { id: '2' },
      { id: '1' },
    ], graph.getNodeById('root'));
    expect(node2.isDestroyed());
    expect(graph.getNodes().length).toBe(3);
    expect(graph.getNodeById('root').get('children').map((node: any) => node.id)).toEqual(['2', '1']);
    ...
    expect(graph.getNodeById('1').get('children').map((node: any) => node.id)).toEqual(['3', '2', '5']);
  });
