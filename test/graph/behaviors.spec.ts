import { Graph, brushSelect, dragCanvas, dragNode, panZoom, showDetails, dragEdge, highlightRelations, multipleSelect } from '../../src';

function createWheelEvent(canvas: HTMLCanvasElement, delta: number, x: number, y: number) {
...
describe('src/behaviors', () => {
  ...
  it('dragNode with delegate after pan & zoom should work', () => {
    ...
    if (shape2) {
      ...
      expect(shape2.getMatrix()).toEqual([1, 0, 0, 1, -5.000000000000001, -5.000000000000001]);
    }
    graph.emit('mousemove', {
      target: node1,
      clientX: 0,
    ...
    if (shape2) {
      expect(shape2.getMatrix()).toEqual([1, 0, 0, 1, -10.000000000000002, -10.000000000000002]);
    }
    graph.getBehavior('dragNode')!.onMouseUp();
  ...
  it('brush select should work', () => {
    ...
    expect(selected.map((node: any) => node.get('id'))).toEqual([
      'node1', 'node2', 'node3' // node1 node2 在 group 里
    ]);
