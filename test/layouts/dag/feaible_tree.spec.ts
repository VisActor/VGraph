...
describe('feasible tree', () => {
  it('creates a tree for a trivial input graph', () => {
    ...
    expect(plainEdge(tree.edges['a-b'])).toEqual(plainEdge(graph.getEdges()[0]));
  });
  it('correctly shortens slack by pulling a node up', () => {
    ...
    expect(plainEdge(tree.edges['a-b'])).toEqual(plainEdge(graph.getEdges()[0]));
    expect(plainEdge(tree.edges['b-c'])).toEqual(plainEdge(graph.getEdges()[1]));
    expect(plainEdge(tree.edges['a-d'])).toEqual(plainEdge(graph.getEdges()[2]));
  });
  it('correctly shortens slack by pulling a node down', () => {
    ...
    expect(tree.edges['b-c']).toBeDefined();
    expect(plainEdge(tree.edges['b-a'])).toEqual(plainEdge(graph.getEdges()[0]));
    expect(plainEdge(tree.edges['b-c'])).toEqual(plainEdge(graph.getEdges()[1]));
  });
