import { Canvas, Rect, Circle, Text, Image, Cubic, Quadratic, Path, Polygon, NodeLayer, GroupLayer } from '../../src';
...
describe('src/event/index.ts', () => {
  ...
  it('get path should work', () => {
    const path = new Path({
      path: [['M', 50, 50], ['H', 150], ['V', 150], ['a', 50, 50, 0, 0, 1, -150, 0], ['L', 50, 150], ['Z']],
      lineWidth: 2,
      strokeStyle: '#666',
    });
    canvas.add(path);
  ...
  it('get polygon should work', () => {
    ...
    canvas.add(hexagram);
    expect(handler.getShapeInLayer({ x: 50, y: 250 + 5 * Math.sqrt(3) }, canvas)).toBe(hexagram);
