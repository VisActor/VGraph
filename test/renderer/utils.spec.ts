import { getCharLen, measureText, getFontByConfigs } from '../../src/renderer/utils/text';
...
import { getArrowBorder, getArrowPoints, isPointOnSimpleArrow, getArrowByLineWidth } from '../../src/renderer/utils/arrow';
import * as IntersectUtil from '../../src/renderer/utils/intersect';

const OS_PLATFORM = process.platform;

function expectValueBySystem(value: number, macValue: number, winValue: number) {
...
describe('src/utils/gradients', () => {
  it('parseLinearGradient should work', () => {
    ...
    const gradient = getGradient('l(0) 0:#fff 0.5:#7ec2f3 1:#1890ff', rect, ctx);
  ...
  it('parseRadialGradient should work', () => {
    ...
    const gradient = getGradient('r(0.5,0.5,0) 0:#fff 0.5:#7ec2f3 1:#1890ff', rect, ctx);
    expect(gradient === rect['r(0.5,0.5,0) 0:#fff 0.5:#7ec2f3 1:#1890ff']);
    const gradient1 = getGradient('l(0) 0:rgb(255, 0, 100) 0.5:blue 1:#1890ff', rect, ctx);
...
describe('src/utils/arrow', () => {
  it('getArrowPoints should work', () => {
    const toPrecision = (points: number[][]) => points.map(([x, y]) => [x.toFixed(10), y.toFixed(10)]); // 保留十位小数，忽略精度丢失
    expect(toPrecision(getArrowPoints(Math.PI / 8, [50 + Math.cos(Math.PI / 8), 50 + Math.sin(Math.PI / 8)], 5, Math.PI / 4))).toEqual(toPrecision([
      [50 - Math.sqrt(8), 50 - Math.sqrt(8)],
      [50, 50],
      [46, 50]
    ]));
    expect(toPrecision(getArrowPoints(Math.PI / 2, [0, 50], 4, Math.PI / 4))).toEqual(toPrecision([
  ...
  it('getArrowBorder should work', () => {
    const toPrecision = (points: number[][]) => points.map(([x, y]) => [x.toFixed(10), y.toFixed(10)]); // 保留十位小数，忽略精度丢失
    expect(toPrecision(getArrowBorder(Math.PI / 8, [50, 50], 4, Math.PI / 4))).toEqual(toPrecision([
      [50 - Math.sqrt(8), 50 - Math.sqrt(8)],
      [50, 50],
      [46, 50]
    ]));
    expect(toPrecision(getArrowBorder(Math.PI / 2, [0, 50], 4, Math.PI / 4))).toEqual(toPrecision([
...
describe('src/utils/intersect', () => {
  ...
  it('rect intersect should work', () => {
    expect(IntersectUtil.isRectIntersect(bbox, {
      left: 50,
      top: 50,
      width: 40,
      height: 40
    })).toBe(false);
    expect(IntersectUtil.isRectIntersect({
      left: 50,
      top: 50,
      width: 40,
      height: 40
  ...
  it('path intersect should work with cubic', () => {
    const cubic = new Cubic({
      strokeStyle: '#ccc',
      points: [[90, 90], [150, 200], [200, 0], [300, 100]],
    });
  ...
  it('getRectIntersect should work', () => {
    ...
    expect(IntersectUtil.getRectIntersect(bbox, [120, 120], 6)).toEqual([64 + 3 * Math.SQRT2, 64 + 3 * Math.SQRT2]);
    expect(IntersectUtil.getRectIntersect(bbox, [120, 0], 8)).toEqual([62 + 4 * Math.SQRT2, 58 - 4 * Math.SQRT2]);
    expect(IntersectUtil.getRectIntersect(bbox, [0, 0], [2, 4, 6, 8])).toEqual([50, 50]);
    expect(IntersectUtil.getRectIntersect(bbox, [120, 0], [2, 4, 6, 8])).toEqual([70, 50]);
    expect(IntersectUtil.getRectIntersect(bbox, [120, 120], [2, 4, 6, 8])).toEqual([64 + Math.SQRT2 * 3, 64 + Math.SQRT2 * 3]);
    expect(IntersectUtil.getRectIntersect(bbox, [0, 120], [2, 4, 6, 8])).toEqual([58 - Math.SQRT2 * 4, 62 + Math.SQRT2 * 4]);
  });
});
