...
import { getEdgeMethods, registerEdge, unRegisterEdge } from '../../src/models/factories';
...
describe('/src/factories/edge.ts', () => {
  ...
  it('turningLine should work', () => {
    ...
    const pathToPrecision = (path: any[][]) =>
      path.map((arr: any[]) => arr.map((elem: any) => (typeof elem === 'number' ? elem.toFixed(10) : elem)));
  ...
  it('bugfix: straight turningLine should work', () => {
    ...
    expect((path as Path).getPointAt(0.5)).toEqual({ x: 101.59154943091895, y: 198.40845056908105 });
  ...
  it('bugfix: autoRotate edge should update background', () => {
    ...
    expect(rect.getMatrix()).toEqual([
      0.7071067906141363, -0.7071067717589586, 0.7071067717589586, 0.7071067906141363, -9.099026474042347,
      128.03300646778357,
    ]);
