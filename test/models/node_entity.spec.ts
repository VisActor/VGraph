...
describe('src/entities/node.ts', () => {
  ...
  it('setState & removeState should work', () => {
    ...
    graph.set('setNodeStateStyles', (state: string, nodeData: any, node: any) => {
      expect(node instanceof Node);
      if (state === 'hover') {
        return {
          strokeStyle: 'red',
  ...
  it('toFront & toBack should work', () => {
    ...
    expect(nodeLayer.children[nodeLayer.children.length - 1]).not.toEqual(n.layer);
    n.toFront();
  ...
  it('temp entities should work', () => {
    ...
    const n1 = graph.add('node', { id: 'tempNode', rank: 2, dummy: true }, true);
    ...
    const e1 = graph.add('edge', { source: 'tempNode', target: 'tempNode1' }, true);
  ...
  it('bugfix: states should resume after updateData()', () => {
    const node = graph.add('node', { id: 'updateNode', label: 'test update node' });
    node.setState('blur');
    expect(node.hasState('blur'));
    expect(node.getKeyShape().get('opacity')).toBe(0.2);
  ...
  it('bugfix: update type should work', () => {
    ...
    const node = graph.add('node', { id: 'updateType1', type: 'rect', width: 100, height: 20 });
