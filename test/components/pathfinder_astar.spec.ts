...
describe('algorithms/PathFinder', function () {
  ...
  it('AStar PathFinder with default options should work', () => {
    ...
    const path1 = compressPath(finder.findPath(startPoint1, endPoint1, gridComponent.gridData));
    const controlPoints1 = convertPath(left, top, step, path1);
    expect(controlPoints1?.length).toBe(2);
    let [col, row] = gridComponent.getIndexByCoord(startPoint1[0], startPoint1[1]);
    ...
    const path2 = compressPath(finder.findPath(startPoint2, endPoint2, gridComponent.gridData));
  ...
  it('AStar PathFinder with lessDiversion should work', () => {
    ...
    const endPoint1 = [155, 105];
    const { left, top, step } = gridComponent.gridData;
    const path1 = compressPath(finder1.findPath(startPoint1, endPoint1, gridComponent.gridData));
    const controlPoints1 = convertPath(left, top, step, path1);
    expect(controlPoints1?.length).toBe(2);
    expect(controlPoints1).toEqual([
      [105, 55],
    ...
    const startPoint2 = [105, 55];
    const endPoint2 = [395, 55];
    const path2 = compressPath(finder1.findPath(startPoint2, endPoint2, gridComponent.gridData));
    const controlPoints2 = convertPath(left, top, step, path2);
    expect(controlPoints2?.length).toBe(6);
    expect(controlPoints2).toEqual([
      [105, 55],
    ...
    const finder2 = new AStarFinder({ allowDiagonal: 'never', lessDiversion: true });
    const startPoint3 = [105, 55];
    const endPoint3 = [155, 105];
    const path3 = compressPath(finder2.findPath(startPoint3, endPoint3, gridComponent.gridData));
    ...
    const path4 = compressPath(finder2.findPath(startPoint4, endPoint4, gridComponent.gridData));
