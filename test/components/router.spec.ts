...
describe('algorithms/PathFinder', function () {
  ...
  it('bugfix: router with non anchor should work right', () => {
    ...
    graph.add('node', {
      id: 'node2', width: 100, height: 30, x: 30, y: 60,
    });
    const edge = graph.add('edge', { source: 'node1', target: 'node2' });
    gridComponent.refresh();
    router.updateEdgePath(edge);
    expect(edge.get('controlPoints')).toEqual([[0, 25], [22.5, 25], [22.5, 45]]);
    const edgeConfigs = edge.getEdgeConfigs();
    expect(edgeConfigs.controlPoints).toEqual([[0, 25], [22.5, 25]]);
  ...
  it('router should work for group', () => {
    ...
    const grid = new Grid(graph, { ignoreGroupTitle: false, step: 10, extraWidth: 5 });
    ...
    graph.data({
      ...
      edges: [
        { source: '1', target: '2', sourceAnchor: 1, targetAnchor: 0 }
      ],
      groups: [
        { id: 'group1', children: ['1', '2'] }
      ]
    });
    const titleBBox = graph.getGroups()[0].titleLayer?.getBBox();
    const [row, col] = grid.getIndexByCoord(titleBBox!.left + 10, titleBBox!.top + 10);
