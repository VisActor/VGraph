import { AddCommand, EdgeEditor, SelectCommand, Stack, UpdateCommand } from '../../src';
...
describe('components/edge_editor', () => {
  let graph: Graph, node1: Node, node2: Node, edgeEditorOptions: EdgeEditorOptions;
  beforeEach(() => {
    ...
    edgeEditorOptions = {
      ...
      shouldTrigger(ev: any, triggerShape: any, node: Node) {
        ...
        if (!triggerShape.get) {
          return triggerShape?.classList.contains('xgraph-react-viewer-anchor');
        }
  ...
  it('EdgeEditor add edge without stack should work.', () => {
    ...
    // 待吸附锚点高亮
    expect(node2.layer.get('__anchors')[0].get('fillStyle')).toBe('#3073F2');
    expect(node2.layer.get('__anchors')[0].get('strokeStyle')).toBe('rgba(48, 115, 242, 0.2)');
  ...
  it('EdgeEditor update edge should work.', () => {
    ...
    const edgeEditor = new EdgeEditor(graph, { ...edgeEditorOptions, stack, editTerminal: true });
    graph.emit('node:mousedown', {
      target: node1,
      relatedTarget: node1.layer.get('__anchors')[0],
      clientX: 100,
  ...
  it('editAnchorStyles should work', () => {
    ...
    new EdgeEditor(graph, {
      ...edgeEditorOptions, stack, editTerminal: true, editAnchorStyles: {
        fillStyle: '#7073F2'
      }
    });
