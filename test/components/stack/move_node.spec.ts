...
import { Grid, MoveNodeCommand, SelectCommand, Stack } from '../../../src/components';
...
describe('/src/commands/move_node', () => {
  ...
  it('move batch nodes should work', () => {
    ...
    expect(stack.stack[2].data.selections.node).toEqual([node1.get('id'), node2.get('id')]);
    expect(stack.stack[2].data.lastPositions).toEqual({ '111': { x: 100, y: 100 }, '222': { x: 450, y: 200 } });
