...
import { Stack, CopyCommand, SelectCommand, PasteCommand } from '../../../src/components';
...
describe('/src/commands/copy', () => {
  ...
  it('copy & paste should work for node', () => {
    ...
    expect(configs.node).toEqual([graph.getNodeById('1').configs, graph.getNodeById('2').configs]);

    stack.execute('paste', { event: e });
    expect(stack.stack.length).toBe(1);
    expect(graph.getNodes().length).toBe(5);
    const node1 = graph.getNodes().filter((node: any) => node.get('x') === 120)[0];
    expect(node1.get('y')).toBe(120);
    const id1 = node1.get('id');

    const node2 = graph.getNodes().filter((node: any) => node.get('x') === 320)[0];
  ...
  it('copy & paste should work for group', () => {
    ...
    const group = graph.getGroups().filter((group: any) => group.get('id') !== 'group')[0];
