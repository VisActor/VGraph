...
describe('src/legend', () => {
    ...
    it('should work with color mapping', () => {
        ...
        // 参数项配置
        const { target, encodeAttr, value, label, orient, rail, color, slide } = legend.options;
        ...
        legend.graphTarget.forEach((item: any) => {
            if (item.get(encodeAttr) >= legend.filterValue[0] && item.get(encodeAttr) <= legend.filterValue[1]) {
    ...
    it('should work with size mapping', () => {
        ...
        expect(legend3.valueDiffer).toBeCloseTo(Math.pow(31, 0.5) - Math.pow(1, 0.5));
    });
});
