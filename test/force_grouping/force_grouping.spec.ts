import { ForceDirectedGrouping, ForceDirectedLayout, Graph, Shape } from '../../src';
...
describe('ForceDirectedGrouping', () => {
  ...
  it('init configure should work and setGetGroupValue should work', (done) => {
    ...
    const shapes = Object.entries(forceGrouping.getAllGroupShapes()).map((d) => d[1]);
    ...
    const fdp = new ForceDirectedLayout({
      ...
      onEnd: () => {
        ...
        expect(Math.abs(forceGrouping.dummyGroupNodes[0].vx)).toBeGreaterThan(0);
        expect(Math.abs(forceGrouping.dummyGroupNodes[0].vy)).toBeGreaterThan(0);
        expect(Math.abs(forceGrouping.dummyGroupNodes[4].vx)).toBeGreaterThan(0);
        expect(Math.abs(forceGrouping.dummyGroupNodes[4].vy)).toBeGreaterThan(0);
        ...
        const nweShapes = Object.entries(forceGrouping.getAllGroupShapes()).map((d) => d[1]);
  ...
  describe('setOptions and config shapeStyle should work', () => {
    ...
    const shapes = Object.entries(forceGrouping.getGroupShapes((v) => v === 0 || v === 3)).map(
      (d) => d[1]
    ) as Shape[];
  ...
  describe('updateData should work', () => {
    ...
    let shapes = Object.entries(fdGrouping.getGroupShapes((v) => v === 0 || v === 3)).map(
      (d) => d[1]
    ) as Shape[];
    expect(shapes.length).toBe(2);
    ...
    shapes = Object.entries(fdGrouping.getGroupShapes((v) => v === 0 || v === 3)).map((d) => d[1]) as Shape[];
    // 1 不存在 只有 3 存在
