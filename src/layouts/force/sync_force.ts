import { ForceBase, ForceCenter, ForceCollision, ForceLink, ForceManyBody, ForceX, ForceY, ForceRadial, InterClusterForce, IntraClusterForce } from './force_func';
...
const defaultForces = (edges: any[], x?: number, y?: number, nodeSize?: number) => {
  return new Map<string, ForceBase>([
    ['link', new ForceLink({ edges, options: { distance: 30 } })],
    ['charge', new ForceManyBody({ options: { strength: -30 } })],
    ['collide', new ForceCollision({ options: { radius: nodeSize ? nodeSize / 2 : 10 } })],
...
export function syncFDP(data: { nodes: NodeData[], edges: EdgeData[] }, options: SyncFDPConfigs) {
  ...
  function moveNodes() {
    ...
    if (configs.autoStop && isConverged(sumMove, maxMove, nodeLen, configs.stopDist)) {
...
function initializeNodes(data: { nodes: NodeData[], edges: EdgeData[] }, configs: SyncFDPConfigs) {
...
function configForces(data: { nodes: NodeData[], edges: EdgeData[] }, configs: SyncFDPConfigs) {
  let forces: Map<string, any>;
  const forceConfigs = configs.forces;
  if (!forceConfigs) {
    forces = defaultForces(data.edges, configs.center!.x, configs.center!.y, configs.nodeSize!);
  } else {
...
function isConverged(sumMove: number, maxMove: number, n: number, minDist = 0.01) {
