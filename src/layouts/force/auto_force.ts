...
import { ForceManyBody, ForceLink, ForceX, ForceY, ForceCenter, ForceCollision } from './';
...
export function autoForces(
  ...
  let strengthXY =
    multi || 0.01 / avgDegree / leaf > 0.05
      ? 0.05
      : 0.01 / avgDegree / leaf;
  // 15 * 4 / 3 = 20
  ...
  const forces: Map<string, any> = autoConfigForces(edges, nodeSize, distance, strengthXY, center);
  const zoomRatio = Math.sqrt(((width * height) / N / 400) * Math.max(avgDegree, 1)) / nodeSize * 1.5 * (options.scaleFactor || 1);
