...
import { isMultiComponentsForGraph, isMultiComponentsForData } from '../utils/connected_components';
...
export function autoFDP(
  ...
  if (!graph) {
    throw new Error('graph is required, you can pass either a Graph instance or a GraphStructure instance');
  }
  let nodes: NodeData[];
  let edges: EdgeData[];
  let connected;
  if (graph instanceof Graph || graph instanceof GraphStructure) {
    nodes = graph.getNodes().map((node: Node | NodeStructure) => node.configs as NodeData);
    edges = graph.getEdges().map((edge: Edge | EdgeStructure) => edge.configs as EdgeData);
