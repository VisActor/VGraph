...
function initTree(
  ...
  if (data.children && !data.collapsed) {
    ...
    data.children.forEach((child: any, i: number) => {
      root.children.push(initTree(child, i, root, rank + 1, heightMap, options));
    });
...
function setLeftThread(t: CompactTree, i: number, cl: CompactTree, modsumcl: number) {
...
function setRightThread(t: CompactTree, i: number, sr: CompactTree, modsumsr: number) {
...
function positionRoot(t: CompactTree, alignParent: 'front' | 'mid' | 'back' = 'mid') {
  // 父节点对齐子节点。
  // mid 居中对齐
  // front 居前对齐
  // back 居后对齐
