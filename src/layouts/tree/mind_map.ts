...
export class MindMap extends LayoutBase {
  ...
  layout(data: TreeData) {
    ...
    const options = this.options as MindMapConfigs & { tab?: number, compact?: boolean, setTreePosition?: (nodeData: TreeData) => { leftTree: TreeData; rightTree: TreeData } };
    ...
    let leftTree: TreeData = { id: data.id, width: data.width, height: data.height, children: [] };
    let rightTree: TreeData = { id: data.id, width: data.width, height: data.height, children: [] };
    ...
    leftTree._offsetY += rightTree.posY + rightTree._offsetY - leftTree.posY - leftTree._offsetY;
...
function layoutX(tree: TreeData, layoutOptions: any, step: -1 | 1, align: -1 | 0 | 1, tab?: number) {
  const children = tree.children;
  const size = getNodeSize(tree, layoutOptions);
