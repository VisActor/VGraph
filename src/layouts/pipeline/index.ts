...
export class PipelineLayout extends LayoutBase {
  ...
  layout() {
    ...
    this.options.coordAssignment !== 'treeLike' && coordSystem.adjust(graph, rankDir);
    ...
    removeDummyNodes(graph, ranks, dummyNodes, gapYs, this.options.ignoreControlPoints);
    this.options.coordAssignment !== 'treeLike' && coordSystem.revert(graph, rankDir);
    ...
    function setCoord(nodeId: string, rank: number) {
      ...
      if (visited[nodeId]) {
        if (rank > node.get('rank')) {
          node.set('rank', rank);
          node.set('x', Math.max(node.get('x'), rank * (nodeSize[0] + rankSep) + x));
          node.targets.forEach((childId: string) => {
  ...
  getAlign(node: Node | NodeStructure, graph: any): number | null {
    ...
    const ys = Object.keys(map).sort((a: string, b: string) => parseInt(a, 10) - parseInt(b, 10));
    if (ys.length) {
