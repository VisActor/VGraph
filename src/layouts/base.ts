...
export interface ILayout {
  ...
  // 设置布局数据
  data: ((data: Graph | GraphStructure | ValidData) => void) | ((data: TreeData | TreeGraph) => void);

  // 获取当前数据
  getData: () => Graph | TreeData | GraphStructure | ValidData;

  // 数据更新
  updateData: ((data: Graph | GraphStructure | ValidData) => void) | ((data: TreeData | TreeGraph) => void);
...
export class LayoutBase {
  ...
  constructor(configs: { graph?: Graph | TreeGraph | GraphStructure } & Record<string, unknown>) {
    this.graph = configs.graph!;
  }

  setOptions(options: { graph?: Graph | TreeGraph | GraphStructure } & Record<string, unknown>) {
    if (options.graph) {
