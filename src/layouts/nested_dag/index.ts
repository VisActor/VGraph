...
export class NestedDAG extends LayoutBase {
  ...
  customLayout: ((graph: Graph | GraphStructure, reuse?: { rank?: boolean; order?: boolean }, rankOnly?: boolean) => void) | undefined;
  ...
  layout(reuse?: { rank?: boolean; order?: boolean }) {
    ...
    if (graph.getNodes().length === 0 && graph.getGroups().length === 0) { // node 和 group 都为空时说明是空数据。直接退出。
      return;
    }
  ...
  buildTree() {
    ...
    for (const group of this.groups) {
      ...
      const padding = normalizePadding(group.get('padding') ?? this.options.padding ?? 20);
    ...
    if (group.children && !group.collapsed) {
      ...
      for (const edge of group.edges) {
        if (visId[edge.source] && visId[edge.target]) {
          ...
          console.error(`error: ${edge.source} or ${edge.target} are not in the same subgraph.`);
        }
      }
      ...
      group.rankKeys = Object.keys(rankMap).sort((a, b) => parseInt(a) - parseInt(b));
    }
  }
    ...
    if (group.children && !group.collapsed) {
      ...
      for (let i = 0; i < length; i++) {
        group.rankMap[keys[i]].forEach((child: any, index: number) => {
          ...
          child.absoluteRank = child.parent.absoluteRank + child.depthCoefficient * child.rank;
      ...
      for (const entity of Object.values(this.entityMap)){
        if (entity.uniqueEdges){
          for (const edge of entity.uniqueEdges){
            if (edge.source === edge.originSource && edge.target === edge.originTarget){
