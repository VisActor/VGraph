...
function getBaryCenter(group: any, direction: 'up' | 'down' = 'down', entityMap: Record<string, any>) {
  let length = 0;
  let sum = 0;
  for (const edge of group.entityEdges) {
    ...
    if (((direction === 'up') && otherRank > absoluteRank)
...
export function sortByBaryCenter(group: any, direction: 'up' | 'down' = 'down', entityMap: Record<string, any>) {
  // 按照 baryCenter 中心排序
  if (!group.children || group.collapsed) {
    return;
  }
  ...
  for (let i = 0; i < length; i++) {
    ...
    const nodesWithBaryCenter = rankEntities.filter((d: any) => d.baryCenter !== undefined).sort((a: any, b: any) => {
