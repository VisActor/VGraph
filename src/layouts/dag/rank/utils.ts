...
import { NodeStructure, EdgeStructure, GraphStructure } from '../../../graph_structure';


export function normalizeRanks(graph: GraphStructure | Graph): void {
  const ranks = graph.getNodes().map((node: Node | NodeStructure) => node.get('rank'));
  const minRanks = Math.min(...ranks);
  graph.getNodes().forEach((node: Node | NodeStructure) => node.set('rank', node.get('rank') - minRanks));
}
