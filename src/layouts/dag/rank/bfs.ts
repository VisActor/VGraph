...
// 前置条件：
// - Graph is a DAG.
export function bfsRank(graph: Graph | GraphStructure, startNode?: Node | NodeStructure) {
  ...
  if (startNode === undefined) {
    ...
    if (startNode === undefined) { startNode = nodes[0]; } // 如果还是没有，则取第一个节点
  }
  const sourceNode = startNode as Node | NodeStructure; // For tslint
  sourceNode.set('rank', 0);
  const setRankUp = (node: Node | NodeStructure, parent: Node | NodeStructure) => {
    if (parent) {
      node.set('rank', parent.get('rank') - 1);
    }
  };
  const setRankDown = (node: Node | NodeStructure, parent: Node | NodeStructure) => {
    if (parent) {
      node.set('rank', parent.get('rank') + 1);
    }
  };
