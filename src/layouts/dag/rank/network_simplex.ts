...
export function initTree(tree: any, graph: Graph | GraphStructure) {
  ...
  tree.getCutValue = (source: string, target: string) => {
    return tree.cutValue[source + '-' + target] ?? tree.cutValue[target + '-' + source];
  };
...
export function cutValue(tree: any, nodeEdges: any, nodeId: string) {
  ...
  let cutVal = 0;
  const edge = tree.getGraphEdge(parent, nodeId) ?? tree.getGraphEdge(nodeId, parent);
  ...
  nodeEdges[nodeId].forEach((e: any) => {
    ...
    if (otherNodeId !== parent) {
      ...
      if (tree.getEdge(nodeId, otherNodeId) !== undefined || tree.getEdge(otherNodeId, nodeId) !== undefined) {
...
export function enterEdge(tree: any, graph: Graph | GraphStructure, edge: any) {
  ...
  graph.getEdges().forEach((e: any) => {
    ...
    if (flip === isDescendant(tree, v, tail) && flip !== isDescendant(tree, w, tail)) {
...
export function exchangeEdges(tree: any, graph: Graph | GraphStructure, edge: any, fEdge: any) {
...
function updateRanks(tree: any, graph: Graph | GraphStructure) {
  ...
  vs.forEach((v: string) => {
    ...
    nodeMap[v].set(
      'rank',
      nodeMap[parent].get('rank') + (fliped ? edge.get('minlen') || 1 : -(edge.get('minlen') || 1))
    );
  });
}
