...
function buildTree(graph: GraphStructure | Graph, dummyNodeSize: number[], rootId?: string) {
  // 从第一层节点开始
  // BFS 建树
  ...
  if (node) {
    ...
    if (rootId === 'xgraphDagFlowRoot' && node.targets.length === 1) {
...
function getDefaultOptions(dagOptions: Partial<DAGLayoutOptions> & { setTreePosition?: (data: TreeData) => { leftTree: TreeData, rightTree: TreeData } }, rootId?: string) {
  return {
    ...dagOptions,
    rankSep: () => dagOptions.rankSep!,
    nodeSep: () => dagOptions.nodeSep!,
    direction: dagOptions.rankDir,
    setTreePosition: dagOptions.setTreePosition ? dagOptions.setTreePosition : (data: TreeData) => {
      return {
        leftTree: { id: data.id, width: data.width, height: data.width, children: [] },
        rightTree: data,
      };
    },
  };
...
export function treeLayoutCoordAssignment(graph: GraphStructure | Graph, dagOptions: Partial<DAGLayoutOptions> & { setTreePosition?: (data: TreeData) => { leftTree: TreeData, rightTree: TreeData } }, rootId?: string) {
  const tree = buildTree(graph, dagOptions?.edgeSep ? [0, dagOptions.edgeSep] : [0, 30], rootId);
