...
// 应该和order放在一起
export function sortLayers(ranks: IRanks, rankClusters: any, direction: 'down' | 'up', bias: boolean) {
  ...
  for (let i = 0; i < rankIndexes.length - 1; i++) {
    ...
    if (rankClusters) {
      ...
      ranks[nextRank] = ranks[nextRank].sort((a: Node | NodeStructure, b: Node | NodeStructure) => {
...
function formNodeClusters(nodes: NodeStructure[]) {
  ...
  for (const cluster of nodeClusters) {
    cluster.sort((a: NodeStructure, b: NodeStructure) => a.get('x') - b.get('x'));
  }
  return nodeClusters;
}
