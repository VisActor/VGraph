...
function preOrder(ranks: any, direction: 'down' | 'up') {
  ...
  for (let i = 0; i < rankIndexes.length - 1; i++) {
    ...
    // 仅为没有_order的节点排序
    if (!ranks[nextRank].some((node: Node | NodeStructure) => (node.get('_order') === undefined))) {
      continue;
    }
    ...
    const nodes = baryCenter.filter((d: any) => d.baryCenter !== undefined).sort((a: any, b: any) => {
  ...
  Object.keys(ranks).forEach((rank: string) => {
    const nodes = ranks[rank].filter((node: Node | NodeStructure) => node.get('_order') !== undefined).sort((a: Node | NodeStructure, b: Node | NodeStructure) => {
...
function dfsInitOrder(ranks: IRanks){
  ...
  Object.keys(ranks).forEach((rank: string) => {
    ranks[rank] = ranks[rank].sort((a: Node | NodeStructure, b: Node | NodeStructure) => {
...
function calculateCrossCount(ranks: IRanks) {
  const rankIndexes = Object.keys(ranks).sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
  let cc = 0;
  for (let i = 0; i < rankIndexes.length - 1; i++) {
...
export function preOrderNestedGraphs(ranks: IRanks, nestGraphs: GraphStructure[], tempEdges: Record<string, any>){
  ...
  nestGraphs.forEach((graph: GraphStructure) => {
    ...
    group.rankKeys = Object.keys(rankMap).sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
    entityMap[group.id] = group;
  });

  const rankIndexes = rankKeys.sort((a, b) => parseInt(a, 10) - parseInt(b, 10));
