...
export function dealDuplicateEdge(
  ...
  if (needFilter) {
    ...
    for (const edge of edges) {
      ...
      if (edgeMap[source + '--and--' + target] || edgeMap[target + '--and--' + source]) {
        const edgeArray = edgeMap[source + '--and--' + target] ?? edgeMap[target + '--and--' + source];
        edgeArray.push(edge);
      } else {
  ...
  for (const edgeArray of repeatEdges) {
    ...
    const { startPoint, endPoint } = edge.getTerminal
      ? edge.getTerminal()
      : {
        startPoint: [nodeMap[edge.get('source')].get('x'), nodeMap[edge.get('source')].get('y')],
        endPoint: [nodeMap[edge.get('target')].get('x'), nodeMap[edge.get('target')].get('y')],
      };
    ...
    if (
      ...
      let { curvePosition = 0.5, curveOffset = 0 } = edge.get('styles') ? edge.get('styles') : {};
      ...
      for (let i = 0; i < length; i++) {
        ...
        // 增加控制点。
        const movement =
          (Math.floor((i + 1 + even) / 2) - 0.5 * even) * direction * (e.get('lineWidth') * scale || scale);
        ...
        if (type === EDGE_TYPES.VLINE) {
          if (Math.abs(dy) > 0) {
            ...
            tempControlPoints = [[(startPoint[0] + endPoint[0]) * 0.5, lineY + movement]];
          }
        } else if (type === EDGE_TYPES.HLINE) {
          if (Math.abs(dx) > 0) {
            tempControlPoints = [
            ...
            tempControlPoints = [[lineX + movement, (startPoint[1] + endPoint[1]) * 0.5]];
          }
        }
...
function getBorderPoints(
  ...
  const { x, y } = pos;
  const box = [x - 0.5 * width, x + 0.5 * width, y - 0.5 * height, y + 0.5 * height];
  const dist = [
 ...
 */
export function getDuplicateEdgeConfigs(edge: Edge, configs: any, count: number, scale = 10) {
  const controlPoints = edge.get('controlPoints');
  const type = edge.get('type');
  const { startPoint, endPoint } = edge.getTerminal();
  const direction = count % 2 === 0 ? -1 : 1;
  const movement = Math.floor((count + 1) / 2) * direction * (edge.get('lineWidth') ?? 1) * scale;
  ...
  configs.controlPoints = configs.source === edge.get('source') ? cps : cps.reverse;
  return configs;
}
