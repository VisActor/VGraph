...
export const attachableDragNode: any = {
  ...
  initTempEdge(target: Node) {
    ...
    const edge = new Edge(({
      source: parent?.get('id') || graph.getNodes().find((node: Node) => node.get('id') !== target.get('id')).get('id'),
      ...
      temp: true,
      ...this.tempEdgeStyles,
    }) as any, graph, graph.edgeContainer);
    if (!parent) {
      edge.hide();
    }
    this.tempEdge = edge;
  ...
  updateNode(offsetX: number, offsetY: number) {
    ...
    if (this.delegate) {
      ...
      this.onDrag(target,);
      !linkAnchor && tempEdge.set('endPoint', [target.configs.x, target.configs.y]);
    }
    this.updateParent();
  ...
  initDelegateShape() {
    ...
    const shape = keyShape.type === 'circle' ? new Circle({
      ...configs,
