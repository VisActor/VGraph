...
export const highlightRelations: any = {
  ...
  highlight(node: Node) {
    ...
    const { blurNodeState, blurEdgeState, activeNodeState, activeEdgeState } = this;
    graph.emit(GRAPH_EVENTS.BATCH_STATE_START, { targets: [node], state: activeNodeState });
    ...
    graph.emit(GRAPH_EVENTS.BATCH_STATE_END, { targets: [node], state: activeNodeState });
  ...
  recover() {
    ...
    graph.emit(GRAPH_EVENTS.BATCH_STATE_START, { targets: [activeNode], state: activeNodeState });
    const autoDraw = graph.disableAutoDraw();
    ...
    graph.emit(GRAPH_EVENTS.BATCH_STATE_END, { targets: [activeNode], state: activeNodeState });
