 ...
 */
export function limitTranslate(graph: Graph | TreeGraph, x: number, y: number, padding?: number, strict = false, bbox?: BBox) {
  if (!bbox) {
  ...
  if (strict && vWidth > width && leftTop.x < left && rightBottom.x > left + width) {
    x = 0;
  }
  if (strict && vHeight > height && leftTop.y < top && rightBottom.y > top + height) {
    y = 0;
  }
