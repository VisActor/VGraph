...
import { batchAdd, batchRemove } from '../../../components/stack/commands/utils';
...
export const PasteAsChildrenCommand = Object.assign({}, CommandBase, {
  ...
  shouldExecute(graph: Graph, args: { event: ClipboardEvent, position?: number[] }) {
    ...
    const rootIds = copyInfo.node.filter((configs: Record<string, any>) => !targets.includes(configs.id));
    // 找不到根节点不能成功复制
    return rootIds.length > 0;
  },

  getSnapshot(graph: Graph, args: { event: ClipboardEvent, parentId: string, edgeConfigs?: Record<string, any>, getNodeConfigs?:(nodeData: any) => Record<string, any>}) {
