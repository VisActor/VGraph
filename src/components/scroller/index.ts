...
export class Scroller extends ComponentBase {
  ...
  constructor(graph: Graph | TreeGraph, options?: ScrollerOptions) {
    ...
    if (this.options.panZoom) {
      graph.addBehavior(
        panZoom,
        typeof this.options.panZoom === 'object' ? Object.assign(this.options.panZoom, defaultOptions) : defaultOptions
      );
    }
  ...
  initScroller() {
    ...
    xScroller.classList.add('xgraph-scroller');
    applyCss(xScroller, {
      left: '0px',
      right: '0px',
      bottom: '4px',
    ...
    xSlider.classList.add('xgraph-slider');
    applyCss(xSlider, {
      position: 'absolute',
      height: `${size}px`,
    ...
    yScroller.classList.add('xgraph-scroller');
    applyCss(yScroller, {
      top: '0px',
      bottom: '0px',
      right: '2px',
    ...
    ySlider.classList.add('xgraph-slider');
    applyCss(ySlider, {
      position: 'absolute',
      width: `${size}px`,
      background: 'rgba(0, 0, 0,0.3)',
  ...
  onMouseDown(e: MouseEvent, orient: 'x' | 'y') {
    this.temps = {
      ...
      offset: orient === 'x' ? parseFloat(this.xSlider.style.left) : parseFloat(this.ySlider.style.top),
      max: orient === 'x' ? this.graph.get('width') : this.graph.get('height'),
      size: orient === 'x' ? parseFloat(this.xSlider.style.width) : parseFloat(this.ySlider.style.height),
      orient,
    };
  }
  ...
  onMouseUp(e: MouseEvent) {
    this.temps = null;
    if (!(e.target as HTMLElement)?.classList.contains('xgraph-slider')) {
      this.hideScroller();
    }
  }
