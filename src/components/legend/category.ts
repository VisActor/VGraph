...
import { Canvas, Layer, Circle, Rect, Text, Path, Quadratic, Cubic, NodeLayer, Icon, Image } from '../../renderer';
import { normalizePadding, applyCss } from '../../utils';
import { ComponentBase } from '../base';
import { getLabelConfigs } from './utils';
import { Entity } from '../../models/entities';
import { CategoryLegendDataItem, CategoryLegendItemStyles, CategoryLegendOptions } from './type';
...
export class CategoryLegend extends ComponentBase {
  ...
  update() {
    ...
    // 更新前选中的 value
    const activeValues = this.clickActiveItem?.map((item: Layer) => item.get('encodeValue'));
    titleLayer.clear();
    nodeLayer.clear();
    if (layer.children[2]) {
  ...
  initShapes() {
    ...
    legendData.forEach((item: any, i: number) => {
      ...
      if (item.label) {
        ...
        if (orient === 'vertical') {
          const maxWidth = Math.min(maxLabelWidth - keyShapeBBox.width - MARKER_GAP, maxLabelWidth);
  ...
  layoutLegend(itemsWidth: number, itemsHeight: number, titleHeight: number) {
    ...
    if (orient === 'vertical') {
      vSep = 0; // 横向间隔为 0;
      vWidth = 0; // 横向平移距离
      hSep = responsive ? hSep : Math.min(Math.max((height - itemsHeight) / intervalNumber, itemsHeight), hSep);
      ...
      if (allHeight > height) {
        ...
        this.canvas.changeSize(this.options.width, allHeight + padding[0] + padding[2]);
      }
    } else {
      ...
      vSep = responsive ? vSep : Math.max((width - itemsWidth) / intervalNumber, vSep);
      ...
      if (allWidth > width) {
        ...
        this.canvas.changeSize(allWidth + padding[1] + padding[3], this.options.height);
      }
    }

    items.forEach((item: any, i: number) => {
      item.translate(padding[3] + vSep * i + vWidth, titleHeight + padding[0] + hSep * i + hHeight);
    });
  }
  ...
  processData() {
    ...
    if (this.options.legendData) {
      this.options.legendData.forEach((item: any) => {
        const legendItem = this.mergeItemConfigs(this.getDefaultItemConfigs(), item, item);
  ...
  mergeItemConfigs(defaultConfigs: any, customConfigs: any, rawData: any) {
    ...
    const marker = Object.assign({}, defaultConfigs.marker, customConfigs.marker);
    const label = customConfigs.label ? customConfigs.label : rawData[encodeAttr]?.toString();
    const value = customConfigs.value || customConfigs.value === 0 ? customConfigs.value : rawData[encodeAttr];
    return {
      marker,
      label,
      value,
  ...
  onClick(target: any) {
    ...
    if (target.get('clickInteraction')) {
      ...
      if (!click.multiple) {
        ...
        this.clickActiveItem = this.clickActiveItem?.filter((item: any) => item !== target);
        this.filterGraph(this.clickActiveItem, 'click');
      }
      this.refresh();
      return;
  ...
  filterGraph(filterItems: any, action: 'hover' | 'click') {
    ...
    const filteredValues = filterItems.map((item: any) => item.get('encodeValue'));
    const encodeAttr = this.options.encodeAttr;
    const { graphActiveState, graphBlurState } = option;
    if (option.filter) {
      graph.filter((entity: Entity) => !filteredValues.includes(entity.get(encodeAttr)), this.options.target);
      ...
      graph.set('emitGraphEvents', true);
      graph.emitEvent(GRAPH_EVENTS.BATCH_STATE_END, { state: graphActiveState });
    }
  }
  ...
  setLegendItemState(interaction: string, e: any, activeState?: string) {
    ...
    let activeStyle: CategoryLegendItemStyles = this.getDefaultActiveState(interaction, e.children[0].configs);
    if (setStyle) {
    ...
    if (activeStyle) {
      ...
      if (activeStyle.textStyles && e.children.length > 1) {
        ...
        if (!text.cacheStyle) {
          text.cacheStyle = this.getCacheStyle(text.configs, activeStyle.textStyles);
        }
  ...
  getDefaultActiveState(interaction: string, legendData: any): CategoryLegendItemStyles {
  ...
  showCurrentPage(page: number) {
    ...
    const max = paginationConfigs.order[page + 1] ? paginationConfigs.order[page + 1] : legendItems.length;
  ...
  initPagination(page: string, sep: number) {
    ...
    const bgRect = new Rect({
      width: orient === 'vertical' ? width : pagination.width + padding[1] + sep,
      height: orient === 'vertical' ? pagination.height + padding[2] + sep : height - this.titleHeight,
      left: orient === 'vertical' ? 0 : width - padding[1] - pagination.width - sep,
      top: orient === 'vertical' ? height - padding[2] - pagination.height - sep : this.titleHeight,
      fillStyle: 'white',
    });
    ...
    const paginationNodeLayer = new NodeLayer({
      ...
      x: orient === 'vertical' ? width / 2 : width - padding[1] - pagination.width / 2,
      y: orient === 'vertical' ? height - padding[2] - pagination.height / 2 : (height + this.titleHeight) / 2,
    });
    ...
    leftArrow.on('click', () => {
      ...
      orient === 'vertical'
        ? parent.translate(0, paginationConfigs.distance[paginationConfigs.curPage])
        : parent.translate(paginationConfigs.distance[paginationConfigs.curPage], 0);
      paginationConfigs.curPage--;
      pageNumber.set('text', `${paginationConfigs.curPage + 1}/${paginationConfigs.count}`);
      if (paginationConfigs.curPage === 0) {
    ...
    [leftArrow, rightArrow].forEach((arrow, i) => {
      arrow.on('mouseenter', (e) => {
        if (i === 0 && paginationConfigs.curPage === 0) {
          return;
        } else if (i === 1 && paginationConfigs.curPage === paginationConfigs.count - 1) {
          return;
        }
