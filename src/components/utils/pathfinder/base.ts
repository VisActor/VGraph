...
export class BaseFinder {
  ...
  getNeighbors(currentNode: { row: number; col: number }, gridData: GridData) {
    ...
    const neighbors = [] as { row: number; col: number; g?: number; h?: number; f?: number; parent?: any }[];
    if (row > 0) {
      if (grid[row - 1][col] === 0) {
        neighbors.push({
          row: row - 1,
  ...
  findPath(startPoint: number[] | number[][], endPoint: number[] | number[][], grid: GridData, startInfos?: any, endInfos?: any) {
