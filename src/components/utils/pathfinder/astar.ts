...
export class AStarFinder extends BaseFinder {
  ...
  findPath(startPoint: number[] | number[][], endPoint: number[] | number[][], gridData: GridData, startInfos?: any) {
    const { left, top, rows, cols, step } = gridData;
    const abs = Math.abs, SQRT2 = Math.SQRT2;
    const weight = 1;
    ...
    while (!openList.empty()) {
      ...
      if (endNodesId.includes(currentNode.id)) { // 找到终点
      ...
      for (const neighbor of neighborNodes) {
        ...
        const ng =
          currentNode.cost! +
          (col - currentNode.col === 0 || row - currentNode.row === 0 ? 1 : SQRT2) +
          (this.lessDiversion ? directionChanged(currentNode, neighbor, endNodes) : 0);
        if (!neighbor.opened || ng < neighbor.cost!) {
          neighbor.cost = ng;
          neighbor.feat = neighbor.feat || weight * Math.min(...endNodes.map((endNode) => heuristic(abs(col - endNode.col), abs(row - endNode.row))));
