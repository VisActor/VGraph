...
export class FisheyePlugin extends ComponentBase {
  ...
  constructor(graph: Graph | TreeGraph, options?: FisheyeOptions) {
    super(graph, options);
    const { distortion, r, inEyeR, showLabel, inEye, outEye, bkgShape } = this.options;
    ...
    if (showLabel && showLabel !== 'none') {
      if (inEye === undefined && outEye === undefined) {
        ...
        console.info('Set custom inEye and outEye, so highlightMethod is ignored.');
        this.inEye = inEye || (() => {});
        this.outEye = inEye || (() => {});
      }
    }
  ...
  initBkgShape() {
    ...
    let shape: Shape;
    const shapeConfigs = { cx: 0, cy: 0, r: 0, fillStyle: '#AAA', opacity: 0.2 };
    if (bkgShape instanceof ShapeBase) {
      shape = bkgShape;
    } else {
      shape = new Circle(bkgShape === true ? shapeConfigs : Object.assign(shapeConfigs, bkgShape));
    }
    shape.capture = false;
  ...
  // canvas
  onTransformed() {
    if (this.shape && this.fisheye !== undefined && this.isEnable && this.options.coordinateSystem === 'canvas') {
      ...
      this.refreshNodes(nodes, this.options.isSetState ?? true, this.options.isScaling ?? false); // 刷新节点渲染属性
      this.graph.refresh();
  ...
  throttleOnMove: (clientX: number, clientY: number) => void = throttle((clientX: number, clientY: number) => {
  ...
  onMove: (clientX: number, clientY: number) => void = (clientX: number, clientY: number) => {
    if (!this.isEnable) {
      return;
    }
    ...
    if (this.shape) {
      this.shape.translate(x - this.shape.getMatrix()[4], y - this.shape.getMatrix()[5]);
      this.shape.set('r', this.fisheye.getInEyeRadius());
    }
