...
export const Fisheye = (
  ...
  function fisheye(d: NodeData & { x: number; y: number }) {
    ...
    return { x: focus[0] + dx * k, y: focus[1] + dy * k, z: Math.min(k, 10), inFisheye };
  }

  fisheye.applyNodes = (nodes: NodeData[], inEye?: (node: NodeData) => void, outEye?: (node: NodeData) => void) => {
    nodes.forEach((node: NodeData) => {
      node.configs.fisheye = fisheye(node.configs);
      if (node.configs.fisheye.inFisheye) {
        if (inEye) { inEye(node); }
      } else {
        if (outEye) { outEye(node); }
      }
    });
  };
