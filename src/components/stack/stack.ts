...
export class Stack {
  ...
  execute(name: string, args: Record<string, unknown> = {}, ignoreSnapshot = false) {
    ...
    const entityCount = selections ? selections.node.length + selections.edge.length + selections.group.length : 0;
    ...
    const newSelections = cmd.execute(input, graph);
    if (newSelections!) {
  ...
  redo() {
    ...
    const newSelections = command.redo(this.getSnapshot(data), graph);
    newSelections! && this.select(newSelections);
    if (this.collab) {
  ...
  select(selections: { node: string[]; edge: string[]; group: string[] }) {
    ...
    graph.emitEvent(GRAPH_EVENTS.BATCH_STATE_START, { targets: entitiesToSelect, state: 'select' });
    ...
    graph.emitEvent(GRAPH_EVENTS.BATCH_STATE_END, { targets: entitiesToSelect, state: 'select', unselect });
  ...
  addCommand(name: string, command: Command) {
    ...
    this.commands[name] = Object.assign(Object.create(Object.getPrototypeOf(command)), command);
  }
  ...
  getSnapshot(data?: Record<string, any>) {
    ...
    if (data) {
      Object.keys(data).forEach((key: string) => {
        ...
        } else if (data[key] instanceof Node || data[key] instanceof Edge || data[key] instanceof Group) {
