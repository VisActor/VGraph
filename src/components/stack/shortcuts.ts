  ...
  return {
    ...
    selectAll: {
      ...
      handler: (e: KeyboardEvent | ClipboardEvent) => {
        ...
        selections = selections.concat(graph.getEdges()).concat(graph.getGroups())
...
export async function execClipboardEvent(configs: { type: 'copy' | 'cut' | 'paste', stack: Stack, stackArgs?: Record<string, unknown>, cmdName?: string }) {
  const { type, stack, stackArgs, cmdName } = configs;
  let success: boolean;
  if (navigator.clipboard) {
    if (type === 'copy' || type === 'cut') {
      const event = new ClipboardEvent(type, { clipboardData: new DataTransfer() });
      ...
      return await navigator.clipboard.readText?.().then((text) => { // firefox 早期版本不支持 clipboard 且也不支持 execCommand('paste')（出于安全性考虑）
        const event = new ClipboardEvent(type, { clipboardData: new DataTransfer() });
  ...
  } else { // 如果浏览器不支持 navigator.clipboard, 降级使用 execCommand
