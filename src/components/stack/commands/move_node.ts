...
export type MoveNodeArgs = {
  ...
  originGroupBox: Record<string, {left: number, top: number, width: number, height: number}>;
...
type MoveNodeSnapshot = {
  ...
  currentGroupBox: Record<string, {left: number, top: number, width: number, height: number}>;
...
export const MoveNodeCommand = Object.assign({}, CommandBase, {
  ...
  getSnapshot(graph: Graph, args: MoveNodeArgs) {
    const edgeIds = args.edgeOriginControlPointsMap ? Object.keys(args.edgeOriginControlPointsMap) : [];
    const edgeNewControlPointsMap = {};
    const currentGroupBox = {};
    edgeIds.forEach((edgeId: string) => {
      const edge = graph.getEdgeById(edgeId);
      edgeNewControlPointsMap[edge.get('id')] = edge.get('controlPoints')?.concat();
    });
...
function batchMoveNode(
  ...
  groupBoxes: Record<string, {left: number, top: number, width: number, height: number}>,
  emitEvent = true
) {
  ...
  emitEvent && graph.emitEvent(GRAPH_EVENTS.MOVE_START, { batch, targets: nodes, target });
  ...
  emitEvent && graph.emitEvent(GRAPH_EVENTS.MOVE_END, { batch, targets: nodes, target });
  graph.enableAutoDraw(autoDraw);
}
