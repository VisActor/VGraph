...
function alignGrid(configs: { x: number, y: number } | Record<string, any> | undefined, step: number) {
...
export const InsertNodeCommand = Object.assign({}, CommandBase, {
  ...
  shouldExecute(graph: Graph, args: InsertNodeArgs) {
    ...
    if ((!graph.getNodeById(source)) || (!graph.getNodeById(target))) {
      console.error(`The source or target of edge ${edge.get('id')} is not exist, insert failed.`);
      return false;
    }
    return true;
  },
  ...
  execute(snapshot: Record<string, any>, graph: Graph) {
    ...
    if (newTargetAnchor !== undefined && newSourceAnchor === newTargetAnchor && newNode.get('anchors')?.length > 1) {
      // 新增的节点从两个不同端口。
