...
export const SelectCommand = Object.assign({}, CommandBase, {
  ...
  execute(snapshot: { selections: (Node | Edge | Group)[] }, graph: Graph | TreeGraph) {
    ...
    graph.emit(GRAPH_EVENTS.BATCH_STATE_START, { targets: selections, state: 'select' });
    ...
    graph.emit(GRAPH_EVENTS.BATCH_STATE_END, { targets: selections, state: 'select', unselect });
