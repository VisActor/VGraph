...
export type HandlerOption = {
  shortcut: string;
  handler: (event: KeyboardEvent | ClipboardEvent, handlerInfo: HandlerOption) => void;
  scope?: string;
};
...
export class Shortcuts {
  ...
  bindKey(key: string, handler: (e: ClipboardEvent, info: HandlerOption) => void, scope = 'all', type?: string) {
  ...
  bindEditKey(type: string, handler: (e: ClipboardEvent, info: HandlerOption) => void, scope = 'all') {
    ...
    for (const handler of this.handlers[key]) {
      if (handler.scope === scope || handler.scope === 'all') {
        ...
        Object.keys(mods).forEach((k: string) => {
          ...
          if ((!mods[k] && handler.mods.indexOf(mod) > -1) || (mods[k] && handler.mods.indexOf(mod) === -1)) {
            modifiersMatch = false;
          }
        });
        if ((handler.mods.length === 0 && !mods[16] && !mods[18] && !mods[17] && !mods[91]) || modifiersMatch) {
