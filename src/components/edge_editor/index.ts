...
export type EdgeEditorOptions = {
   ...
   * 判断是否可触发连线编辑
   */
  shouldTrigger?: (e: GraphEvent, shape: Shape, target: Node, edge?: Edge) => boolean;
  /**
   ...
   * 判断是否可完成编辑
   */
  shouldDrop?: (source: Node, target: Node, sourceAnchor: number, targetAnchor: number, edit: boolean) => boolean;
  /**
   ...
   * 在拖拽过程中，哪些锚点应当显示
   */
  showAnchors?: (anchorConfigs: AnchorConfigs, anchorShape: Layer, node: Node, fromNode: Node) => boolean;
  /**
   ...
   * 自定义新增/编辑的连线配置
   */
  getEdgeConfigs?: (configs: Record<string, string>, edit: boolean) => Record<string, any>;
};
...
export class EdgeEditor extends ComponentBase {
  ...
  onMouseDown(ev: any) {
    ...
    if (originEdge) {
      ...
      const type = this.state.target === originEdge.source ? 'source' : 'target';
      const anchorIndex = Number(
        triggerShape?.get
          ? triggerShape.get('anchorIndex')
          : triggerShape?.closest(DOM_ANCHOR_SELECTOR)?.getAttribute('anchorindex')
      );

      if (
        (target !== originEdge.source && target !== originEdge.target) || // 不是连线上的节点不可编辑
        // 连接锚点不匹配不可编辑
        (originEdge.get(`${type}Anchor`) !== anchorIndex && originEdge.get(`__${type}`) !== anchorIndex)
      ) {
  ...
  onEdgeMouseDown(ev: GraphEvent) {
    ...
    if ((x - startPoint[0]) ** 2 + (y - startPoint[1]) ** 2 < (x - endPoint[0]) ** 2 + (y - endPoint[1]) ** 2) {
  ...
  onMouseMove(ev: GraphEvent) {
    ...
    if (!this.state.dragging) {
      if (refresh) {
        if (this.options.shouldTrigger!(ev, triggerShape!, this.state.target!, this.state.edge!)) {
          ...
          if (!this.state.edge || !this.options.editTerminal) {
            ...
            // 编辑连线
            this.state.anchorType = this.state.target === edge.source ? 'source' : 'target'; // 自环有问题
            ...
            edge.configs[`origin${this.state.anchorType}Anchor`] = edge.configs[`${this.state.anchorType}Anchor`];
            edge.configs[`origin${this.state.anchorType}`] = edge.configs[this.state.anchorType];
    ...
    if (this.options.autoTranslate) {
      ...
      const targetPoint = this.state.anchorType === 'source' ? 'startPoint' : 'endPoint';
      this.clearInterval();
  ...
  refreshMagnet(point: { x: number; y: number }) {
    ...
    const result = getMagnetAnchor(point, target!, this.graph, magnetDist, group);
    if (!result || !shouldMagnet(result.node!, (result.node! as Node).get('anchors')[result.anchorIndex], target)) {
  ...
  update(ev: GraphEvent) {
    ...
    if (this.router) {
      const ignoreMinDist = this.state.anchorType === 'source' ? { start: true } : { end: true };
      this.router.updateEdgePath(edge, undefined, this.state.lastTarget ? {} : ignoreMinDist);
    }
    edge.updatePosition();
  ...
  onMouseUp(ev: GraphEvent) {
    ...
    const nodeAnchorIndex = point
      ? IntersectUtil.getNearestPoint(lastTarget?.getAnchorPositions?.() || [[]], point)
      : undefined;
    const targetAnchor = magnetAnchor ? magnetAnchor.anchorConfigs.index : nodeAnchorIndex;
    ...
    if (this.state.dragging && target) {
      ...
      if (
        (this.state.anchorType === 'target' &&
          !this.options.shouldDrop!(target, lastTarget!, edge!.get('sourceAnchor'), targetAnchor!, this.state.edit)) ||
        (this.state.anchorType === 'source' &&
          !this.options.shouldDrop!(lastTarget!, target, targetAnchor!, edge!.get('targetAnchor'), this.state.edit))
      ) {
  ...
  recoverEditedEdge(edge: Edge, select = true) {
    ...
    const { anchorType } = this.state;
    if (!this.options.editTerminal) {
      console.error(`Return from recover edited edge with source: ${edge.get('source')} & target: ${edge.get('target')}`);
      return;
    }
    edge.configs[`${this.state.anchorType}Anchor`] = edge.configs[`origin${this.state.anchorType}Anchor`];
    edge.configs[this.state.anchorType] = edge.configs[`origin${this.state.anchorType}`];
    ...
    delete edge.configs[this.state.anchorType === 'source' ? 'startPoint' : 'endPoint'];
  ...
  setEdgeState(target: Edge) {
    ...
    if (target.hasState('select')) {
      ...
      sourceNode.get('anchors')?.[__source] && (sourceNode.get('anchors')[__source].visible = true);
      targetNode.get('anchors')?.[__target] && (targetNode.get('anchors')[__target].visible = true);
