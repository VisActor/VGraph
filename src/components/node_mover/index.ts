...
export class NodeMover extends ComponentBase {
  ...
  stackArgs = {
    ...
    originGroupBox: {} as Record<string, { left: number, top: number, width: number, height: number }>,
    edgeOriginControlPointsMap: {} as Record<string, number[] | null | undefined>, // <id, controlPoints>
  };
  constructor(graph: Graph, options: NodeMoverOptions) {
    ...
    if (options.snapline !== false) {
      this.snapline = new Snapline(graph, options.snapline === true ? {} : options.snapline);
    }
    insertStyles(`
    .${GRABBING_CLS} {
  ...
  snapshot() {
    ...
    function getEdgesSnapshot(entity: Node | Group) {
      for (const edge of entity.edges) {
        if (!visit[edge.get('id')]) {
          ...
          edges.push(edge);
          stackArgs.edgeOriginControlPointsMap[edge.get('id')] = edge.get('controlPoints')?.map((d: number[]) => [d[0], d[1]]);
        }
      }
    }
    ...
    this.state.debounce = this.options.debounce === true || (!!this.router && (edges.length >= 15 || this.graph.getNodes().length > 200));
  }
  ...
  onMouseMove(ev: GraphEvent) {
    ...
    if (!this.state.dragging) {
      if (refresh) {
        ...
        if (this.options.shouldTrigger(ev, this.state.triggerShape)) {
          ...
          graph.emitEvent(GRAPH_EVENTS.MOVE_START, { batch: this.isBatch, targets: this.state.nodes });
    ...
    if (this.options.autoTranslate && !(ev as any).mock) {
      this.clearInterval();
      this.state.interval = autoTranslate(graph, this.state.shapeBox, (x: number, y: number) => {
  ...
  updateNodes(offsetX: number, offsetY: number) {
    ...
    if (this.options.group) {
      ...
      graph.getGroups().forEach((group: Group) => {
        if (groupMap[group.get('id')]) { // 正被拖拽的group，不处理
          return;
        }
  ...
  onMouseUp(ev: GraphEvent) {
    ...
    if (this.state.dragging) {
      ...
      this.graph.enableAutoDraw(autoDraw);
      graph.emitEvent(GRAPH_EVENTS.MOVE_END, { batch: this.isBatch, targets: nodes });
