...
import { cloneDeep, isGradient, translate, rotate, scale, changedMatrix, pointMultiply } from '../utils/';
...
export interface IShape {
  ...
  get: (k: string) => any;
  // 设置属性值
  set: ((k: string, v: unknown) => IShape) & ((data: Record<string, unknown>) => IShape);
...
export class ShapeBase extends EventEmitter implements IShape {
  ...
  setConfig(key: string, value: unknown) {
    ...
    if (!configs[key] || configs[key] !== value) {
      ...
      if (parent && [
        ...
        'cx', 'cy', 'r',
        'x', 'y', 'textAlign', 'textBaseline'].includes(key)) {
        parent.bbox = null;
      }
  ...
  getBBox(): BBox {
    ...
    const minLeft = Math.min(leftTop.x, leftBottom.x, rightTop.x, rightBottom.x);
    const minTop = Math.min(leftTop.y, leftBottom.y, rightTop.y, rightBottom.y);
    const maxLeft = Math.max(leftTop.x, leftBottom.x, rightTop.x, rightBottom.x);
  ...
  shouldDraw(bbox: BBox = { left: 0, top: 0, width: 0, height: 0 }) {
    return this.visible && (this.configs.opacity === undefined || this.configs.opacity !== 0);
  }
