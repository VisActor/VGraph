...
import { ARROW_LINE_WIDTH, getArrowByLineWidth, getArrowPoints } from '../../utils/arrow';
...
export default {
  rectPath(ctx: any, shape: ShapeBase) {
    ...
    ctx.arc(right - drawRadius[1], top + drawRadius[1], drawRadius[1], -Math.PI / 2, 0, false);
    // 右下
    ctx.lineTo(right, bottom - drawRadius[2]);
    ctx.arc(right - drawRadius[2], bottom - drawRadius[2], drawRadius[2], 0, Math.PI / 2, false);
    // 左下
    ctx.lineTo(left + drawRadius[3], bottom);
    ctx.arc(left + drawRadius[3], bottom - drawRadius[3], drawRadius[3], Math.PI / 2, Math.PI, false);
    // 左上
    ctx.lineTo(left, top + drawRadius[0]);
    ctx.arc(left + drawRadius[0], top + drawRadius[0], drawRadius[0], Math.PI, Math.PI * 3 / 2, false);
  },
  ...
  pathPath(ctx: any, shape: any) {
    ...
    let startPoint = p0;
    let endPoint = lastSegment.configs.points[lastSegment.configs.points.length - 1];
    // 对于 cubic 和 quadratic 的 endPoint 不是 p3，所以之前的写法错误

    // TODO arc 不应该按直线距离去缩减，应缩减对应角度。由于实现复杂 && 粗椭圆弧线箭头不多见，现在暂时不处理
    if (startArrow) {
      const tempPoint = drawShapeStartArrow(ctx, shape, [p0.x, p0.y], [p0.x, p0.y]);
    ...
    if (endArrow) {
      const tempPoint = drawShapeEndArrow(ctx, shape, [endPoint.x, endPoint.y], [endPoint.x, endPoint.y]);
  ...
  drawSinglePath(ctx: any, segment: any, endPoint?: any) {
    ...
    switch (segment.type) {
      ...
      case 'arc':
        ...
        break;
      case 'cubic':
        ctx.bezierCurveTo(points[1].x, points[1].y, points[2].x, points[2].y, lastPoint.x, lastPoint.y);
        break;
      case 'quadratic':
        ctx.quadraticCurveTo(points[1].x, points[1].y, lastPoint.x, lastPoint.y);
        break;
      default:
        break;
    }
  ...
  textPath(ctx: any, shape: any) {
    const { x, y, text, fontSize, textBaseline, fillStyle, strokeStyle } = shape.configs;
    if (text === '') {
      return;
    }
...
function drawShapeStartArrow(ctx: any, shape: any, startPoint: number[], point: number[]) {
...
function drawShapeEndArrow(ctx: any, shape: any, endPoint: number[], point: number[]) {
  ...
  if (lineWidth > 1 || ctx.globalAlpha !== 1) {
    endPoint = [
      point[0] - endRad.x * height,
      point[1] - endRad.y * height];
  }
...
function drawArrow(ctx: any, arrowConfigs: ArrowType, angle: number, p1: number[], arrowLength: number, arrowAngle: number) {
  ctx.setLineDash([]);
  ...
  if (typeof arrowConfigs !== 'boolean' && arrowConfigs.type === 'default-round') {
    ctx.lineJoin = 'round';
  }
