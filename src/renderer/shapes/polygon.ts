...
import { calculateRegularNPolygonByEdgeLength, calculateRegularNPolygon, calculateNPointStar } from '../utils/polygon';
...
export class Polygon extends ShapeBase {
  ...
  setConfig(key: string, value: any): void {
    ...
    if (['type', 'n', 'cx', 'cy', 'edgeLength', 'outerRadius', 'innerRadius'].includes(key)) {
  ...
  if (configs.type && configs.n) {
    ...
    switch (configs.type) {
      case 'regular':
        ...
        break;
      case 'star':
        points = configs.outerRadius ? calculateNPointStar(configs.n, configs.outerRadius, configs.innerRadius) : [];
        break;
      default:
        break;
    }
    points = points.map(([x, y]) => [configs.cx ? x + configs.cx : x, configs.cy ? y + configs.cy : y]);
  }
  return points;
}
