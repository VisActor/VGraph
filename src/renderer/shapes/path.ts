...
export class Path extends ShapeBase {
  ...
  getPointAt(t: number): Point {
    ...
    if (t === 0) {
      ...
      t = 1 - (currentLen + pointSegment.length - pointLen) / pointSegment.length;
    }
    if (pointSegment.type === 'line') {
      ...
      const [, cx, cy, rx, ry, theta, dTheta, psi] = pointSegment.configs.params;
      t = dTheta * t;
      return {
      ...
      const points = pointSegment.configs.points;
      return {
        x: cubicUtil.getDimAt(t, points[0].x, points[1].x, points[2].x, points[3].x),
        y: cubicUtil.getDimAt(t, points[0].y, points[1].y, points[2].y, points[3].y),
      };
