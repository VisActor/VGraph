...
export default class EdgeLayer extends LayerBase {
  ...
  shouldDraw(bbox: BBox): boolean {
    ...
    if (sourceBox.left + sourceBox.width < left && targetBox.left + targetBox.width < left) {
      return false;
    }
    if (sourceBox.top > top + height && targetBox.top > top + height) {
      return false;
    }
    if (sourceBox.top + sourceBox.height < top && targetBox.top + targetBox.height < top) {
      return false;
    }
    return true;
  }
  ...
  getBBoxForHit(): BBox {
    ...
    // 如果有箭头，则扩展bbox判定范围
    if (startArrow) {
      arrowLength = Math.max(arrowLength, getArrowByLineWidth(lineWidth, startArrow).length);
    }
    if (endArrow) {
      arrowLength = Math.max(arrowLength, getArrowByLineWidth(lineWidth, endArrow).length);
    }
