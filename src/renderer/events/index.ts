...
export default class EventManager {
  ...
  emitEventWithoutBubble(eventType: string, event: MouseEvent, shape: ShapeBase) {
    if (shape) {
      const e = this.getEvent(eventType, event, shape);
  ...
  getLayerParent(shape: any): LayerBase {
    let target = shape;
    while (target) {
      if ([LAYER_TYPES.NODE, LAYER_TYPES.EDGE, LAYER_TYPES.GROUP].includes(target.type)) {
        break;
      }
      target = target.getParent();
    }
