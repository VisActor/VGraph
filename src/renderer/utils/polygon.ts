...
export function calculateNPointStar(
  ...
  const angle = Math.PI / n;
  if (!innerRadius) { innerRadius = (outerRadius * Math.cos(2 * angle)) / Math.cos(angle); }
  const points = [];
  for (let i = 0; i < n; i++) {
    points.push([
...
export function isPointInPolygon(point: number[], polygon: number[][]) {
  ...
  for (let i = 0, j = n; i < n; j = i++) {
    const p1 = polygon[i];
    const p2 = polygon[j];
    if (onSegment(p1, p2, point)) { return true; } // 点在线段上
    if (onLeft(p1, p2, point)) { inPoligon = !inPoligon; }
  }
  return inPoligon;
}
...
export function isPointInPolygonStroke(
  ...
  for (let i = 0, j = n; i < n; j = i++) {
    ...
    if (getLineDist({ x: point[0], y: point[1] }, x1, y1, x2, y2) <= lineWidth) { return true; }
  }
}
