...
export function getDimAt(t: number, p0: number, p1: number, p2: number): number {
  const nt = 1 - t;
...
export function getLength(p0: number[], p1: number[], p2: number[], iteration: number): number {
  let px = p0[0];
  let py = p0[1];
...
export function getProjectionDist(target: number[], p0: number[], p1: number[], p2: number[], out: number[] = []): number {
  let t = 0;
  let interval = 0.005;
  let d = Infinity;
...
export function drawPath(ctx: CanvasRenderingContext2D, p0: number[], p1: number[], p2: number[]) {
  ctx.beginPath();
