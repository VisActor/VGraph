...
export function getCircleIntersect(bbox: BBox, point: number[]): [number, number] | null {
  const [x, y] = point;
...
export function getRectIntersect(bbox: BBox, point: number[], radius?: number | number[]) {
  ...
  if (radius) {
    ...
    let center: number[] = [];
    let rad = null;
    if (radius[0] > RADIUS_IGNORE_THRESHOLD && points[0] < minX + radius[0] && points[1] < minY + radius[0]) {
      // 左上
      center = [minX + radius[0], minY + radius[0]];
      rad = radius[0];
    } else if (radius[1] > RADIUS_IGNORE_THRESHOLD && points[0] > maxX - radius[1] && points[1] < minY + radius[1]) {
      // 右上
      center = [maxX - radius[1], minY + radius[1]];
      rad = radius[1];
    } else if (radius[2] > RADIUS_IGNORE_THRESHOLD && points[0] > maxX - radius[2] && points[1] > maxY - radius[2]) {
      // 右下
      center = [maxX - radius[2], maxY - radius[2]];
      rad = radius[2];
    } else if (radius[3] > RADIUS_IGNORE_THRESHOLD && points[0] < minX + radius[3] && points[1] > maxY - radius[3]) {
      // 左下
