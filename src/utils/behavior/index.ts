...
export function detectTrackPad(e: any) {
  ...
  // mac 触摸板
  if ((wheelDeltaY === -3 * deltaY ||
    wheelDeltaY === -6 * deltaY ||
    (Number.isInteger(wheelDeltaY) && Number.isInteger(deltaY))
    && (wheelDeltaY % 120 !== 0 && wheelDeltaY % 180 !== 0))) {
    return true;
  }
  // 240 是 Mac 的滚轮滚动距离
  // 180 是 Win 的滚轮滚动距离
  ...
  // windows 触摸板
  if (Math.abs(wheelDeltaY) <= Math.abs(deltaY) && Math.abs(deltaY) !== 1.25
    && (wheelDeltaY % 120 !== 0 && wheelDeltaY % 180 !== 0)) {
    return true;
  }
...
export function getMagnetAnchor(point: { x: number, y: number }, source: Node, graph: Graph | TreeGraph, magnetDist = 28, group = false) {
  ...
  entities.forEach((node: Node | Group) => {
    ...
    if (node.layer.shouldDraw(viewport)) {
      ...
      positions.forEach((pos: number[], i: number) => {
        ...
        const dist = (pos[0] - point.x) * (pos[0] - point.x) + (pos[1] - point.y) * (pos[1] - point.y);
        if (dist < minDist) {
          minDist = dist;
          minNode = node;
          minAnchorIndex = i;
