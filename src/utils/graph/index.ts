...
export function normalizePadding(padding: number | number[]): [number, number, number, number] {
...
export function resizeToExport(
  ...
  graph.resetMatrix();
  const padding = options?.padding ? normalizePadding(options!.padding) : [0, 0, 0, 0];
  const bbox = graph.getGraphBBox();
  const width = Math.min(options?.maxWidth || 10000, bbox.width) + padding[1] + padding[3];
  const height = Math.min(options?.maxHeight || 1000, bbox.height) + padding[0] + padding[2];
