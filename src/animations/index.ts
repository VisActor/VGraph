import { easeLinear, easeCubic, easePoly, easeQuad, easeSin, easeExp, easeBounce } from 'd3-ease';
...
export default class Animation {
  ...
  execute(time: number, animator: Animator): boolean {
    ...
    let ratio: number;
    if (animator.repeat) {
      if (typeof animator.repeat === 'number' && elapsed / duration > animator.repeat) {
  ...
  addAnimatorToEnd(configs: AnimationConfigs) {
    const animator: Animator = Object.assign({}, DEFAULT_ANIMATE_CONFIGS, configs);
    animator.interpolates = {};
    if (!animator.onFrame) {
      const target = configs.target;
      Object.keys(animator.configs).forEach(k => {
        // TODO 支持 transform
        animator.interpolates[k] = interpolate(target.get(k), animator.configs[k]);
      });
    }
    animator.startTime = this.time;
  ...
  removeAnimator(id: string) {
    const index = this.queue.findIndex((configs: Animator) => configs.id === id);
    if (index >= 0) {
