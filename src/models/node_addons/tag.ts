...
// https://www.figma.com/file/9DZW5QHmVjGczKVXrIzWkP/rq-%E5%9B%BE%E8%B0%B1demo?node-id=1057%3A55278&t=Uxj7AGE2lVmenb0P-0
export function initTag(container: Layer, options: SingleTagOptions): Layer {
  const { left, top, text, maxWidth, theme, label = {}, background = {}, icon, close } = options;
  ...
  const rectShape = new Rect({
    ...
    fillStyle: '#F0F1F3',
    width: textWidth + (iconSize ? iconSize + 4 : 0) + (closeSize ? closeSize + 4 : 0) + padding[1] + padding[3],
    height: rectHeight,
    ...background,
  });
