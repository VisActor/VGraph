...
type NodeEntityConfigs = NodeConfigs & { id: string; x: number; y: number; width: number; height: number };
...
export class Node extends Entity implements INode {
  ...
  constructor(configs: NodeConfigs, graph: any, container: Layer, batching = false, temp = false) {
    super(configs, graph, container, temp);
    if (temp || graph.get('renderMode') !== 'canvas') {
      this.temp = true;
      if (configs.anchors && !Array.isArray(configs.anchors[0])) {
        (configs.anchors as AnchorConfigs[]).forEach((anchor: AnchorConfigs, i: number) => {
  ...
  protected convertToAbsolutePosition(relativePosition: number[] | number): number[] {
    ...
    return [(width - lineWidth) * (relativePosition[0] - 0.5), (height - lineWidth) * (relativePosition[1] - 0.5)];
  }
   ...
   */
  showAnchors(
    filter?: (anchorConfigs: AnchorConfigs, anchor: Layer, node: Node, fromNode?: Node | Group) => boolean,
    node?: Node | Group
  ) {
  ...
  updateData(data: NodeConfigs | undefined, refreshGroup = true) {
    ...
    if (
      ...
      if (
        ...
        (newConfigs.anchors as AnchorConfigs[]).forEach((anchor: AnchorConfigs, i: number) => {
    ...
    if (newConfigs.type !== type || this.layer.children.length === 0) {
      this.layer.clear();
      this.keyShape = getNodeMethods(newConfigs.type).init(this.layer, newConfigs);
      if (newConfigs.icons) {
   ...
   * 是否忽略更新连接边的位置。
   */
  updatePosition(x: number = this.configs.x, y: number = this.configs.y, ignoreEdge = false) {
    const configs = this.configs;
    configs.x = x;
    configs.y = y;
    this.layer.set({ x, y });
  ...
  getAnchorPositions(): [number, number][] {
    ...
    if (Array.isArray(anchorConfigs[0])) {
      ...
      relativePositions = anchorConfigs.map((shapeCfgs: AnchorConfigs) => shapeCfgs.position);
      offsets = anchorConfigs.map((shapeCfgs: AnchorConfigs) => shapeCfgs.offsets);
      linkOffsets = anchorConfigs.map((anchorConfigs: AnchorConfigs) => anchorConfigs.linkOffsets);
    }
  ...
  getAnchorRelativePositions(): number[][] {
    ...
    return anchorConfigs.map((anchorConfig: AnchorConfigs | number[]) => {
      ...
      return [width * (position[0] - 0.5) + offsets[0], height * (position[1] - 0.5) + offsets[1]];
    });
  }
   ...
   * 计算出的连接点。
   */
  getLinkPoint(point: number[], anchorIndex?: number, edge?: Edge, linkType?: 'source' | 'target'): [number, number] {
    ...
    if (isNaN(center[0]) || isNaN(center[1])) {
      this.graph.throw(`Invalid node coord, id: ${this.get('id')}, x: ${this.get('x')}, y: ${this.get('y')}`);
      return [0, 0];
    }
    if (isNaN(this.get('width')) && isNaN(this.get('r'))) {
      this.graph.throw(`Invalid node width, id: ${this.get('id')}, width: ${this.get('width')}`);
      return [0, 0];
    }
    ...
    return IntersectUtil.getRectIntersect(this.getBBox(), point, radius) || center;
  }
