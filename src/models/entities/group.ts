...
import { BBox, GroupLayer, Layer, Rect, Text, ShapeBase, Icon, IntersectUtil } from '../../renderer';
import { normalizePadding } from '../../utils';
import { AnchorConfigs, GroupConfigs, GroupTitleConfigs } from '../../typings/model';
...
export class Group extends Entity {
  ...
  constructor(configs: GroupConfigs, graph: any, container: Layer, temp?: boolean) {
    ...
    if (configs.titleSize) {
      this.titleWidth = this.get('titlePosition') === 'left' ? configs.titleSize : 0;
      this.titleHeight = this.get('titlePosition') === 'left' ? 0 : configs.titleSize;
    }
  }
  ...
  // https://www.figma.com/file/9DZW5QHmVjGczKVXrIzWkP/%F0%9F%93%88rq-xGraph%E5%9B%BE%26%E8%A7%84%E8%8C%83?type=design&node-id=3067-95672&mode=design&t=5CAvvAw61KrMfPiN-4
  private initTitle(configs: GroupTitleConfigs, width: number, height: number) {
    ...
    let textWidth = titlePosition === 'left' ? bkgSize - 24 : width - padding[1] - padding[3];
    const textHeight = titlePosition === 'left' ? height - padding[0] : bkgSize - padding[0];
    if (bkgConfigs) {
    ...
    if (configs.icon) {
      ...
      textWidth -= size + 4;
      const icon = new Icon({
        x: titlePosition === 'left' ? bkgSize - 12 - size / 2 : width - padding[1] - size / 2,
  ...
  private getBkgSize() {
    ...
    return this.get('titlePosition') === 'left' ? DEFAULT_TITLE_WIDTH : DEFAULT_TITLE_HEIGHT;
  }

  private initTitleBackground(bkgConfigs: any, width: number, height: number, size: number) {
    ...
    if (typeof radius === 'number') {
      configs.radius = titlePosition === 'left' ? [radius, 0, 0, radius] : [radius, radius, 0, 0];
    }
  ...
  updateData(configs: Partial<GroupConfigs>) {
    ...
    const defaultConfigs = this.getDefaultConfigs('setDefaultGroup', newConfigs);
    ...
    } else if (this.titleLayer && !(newConfigs.title || newConfigs.renderGroupTitle)) {
      this.destroyTitle();
      this.titleHeight = 0;
      this.titleWidth = 0;
    }
  ...
  private getAnchorDrawPositions(): [number, number][] {
    ...
    if (Array.isArray(anchorConfigs[0])) {
      ...
      relativePositions = (anchorConfigs as AnchorConfigs[]).map((shapeCfgs: AnchorConfigs) => shapeCfgs.position);
      offsets = (anchorConfigs as AnchorConfigs[]).map((shapeCfgs: AnchorConfigs) => shapeCfgs.offsets || [0, 0]);
    }
    const { left, top, width, height } = this.getBBox();
    // 宽高可能未定义，避免 NaN.
    return relativePositions.map((pos: number[], i: number) => {
      const offset = offsets[i] || [0, 0];
      return [left + width * pos[0] + offset[0] || 0, top + height * pos[1] + offset[1] || 0];
    });
  }
   ...
   */
  getLinkPoint(point: number[], anchorIndex?: number, edge?: Edge, linkType?: 'source' | 'target') {
    ...
    return IntersectUtil.getRectIntersect(this.background!.getBBox(), point, this.background!.get('radius')) || center;
  }
  ...
  isAnchorConnected(index: number, type?: 'source' | 'target') {
    ...
    for (const edge of edges) {
      if (type !== 'target' && edge.source === this && edge.get('__source') === index) {
        return true;
      }
      if (type !== 'source' && edge.target === this && edge.get('__target') === index) {
        return true;
      }
    }
    return false;
   ...
   * 是否在移除子实体后刷新分组的边界框。
   */
  removeChild(entity: Node | Group, destroy = true, refresh = true, removeChild = true) {
  ...
  refreshBox(forced?: boolean) {
    ...
    if (
      ...
      if (titleLayer) {
        ...
        titleLayer.setMatrix([1, 0, 0, 1, bbox.minX - this.titleWidth, bbox.minY - this.titleHeight]);
      }
  ...
  getPadding(): [number, number, number, number] {
    const configs = this.configs;
    return configs.padding !== undefined ? normalizePadding(configs.padding) : [20, 20, 20, 20];
  }
  ...
  getInnerBox() {
    const padding = this.getPadding();
    const { fixLeft, fixTop, fixWidth, fixHeight, collapsed, titlePosition } = this.configs;
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
    ...
    box.maxX = (isNaN(fixWidth!) ? maxX : fixWidth! + box.minX + padding[3]) + padding[1];
    box.maxY = (isNaN(fixHeight!) ? maxY : fixHeight! + box.minY + padding[0]) + padding[2];
  ...
  protected initAnchorShapes(anchorConfigs: AnchorConfigs[]) {
    ...
    anchorConfigs.forEach((anchor: AnchorConfigs, i: number) => {
      ...
      if (anchor.type || anchor.setStyles) {
        ...
        const shape = anchorMethod.init(layer, anchor, this.configs, anchorPositions[i]);
   ...
   */
  showAnchors(
    filter?: (anchorConfigs: AnchorConfigs, anchor: Layer, group: Group, fromNode?: Node | Group) => boolean,
    node?: Node | Group
  ) {
    const anchors = this.get('anchors');
    const anchorShapes = this.layer.get('__anchors');
   ...
   * 设置分组及其子节点的 shutoff 状态。 一般仅 xGraph 内部使用。
