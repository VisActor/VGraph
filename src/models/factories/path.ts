  ...
  switch (position) {
    case 'left':
      return [
        ...
        ['Q', startX - dist, startY, startX - dist, startY + radius, qPathLength],
    ...
    case 'bottom':
      return [
        ...
        ['Q', startX, startY + dist, startX + radius, startY + dist, qPathLength],
...
export function getArcLoopPath(dist: number, edgeConfigs: any) {
  ...
  switch (position) {
    case 'left':
      quadraticPoints.push([startPoint[0] - dist, (startPoint[1] + endPoint[1]) / 2]);
      break;
    case 'right':
      quadraticPoints.push([startPoint[0] + dist, (startPoint[1] + endPoint[1]) / 2]);
      break;
    case 'bottom':
      quadraticPoints.push([(startPoint[0] + endPoint[0]) / 2, endPoint[1] + dist]);
      break;
    default:
      quadraticPoints.push([(startPoint[0] + endPoint[0]) / 2, endPoint[1] - dist]);
  }
...
export function getRouterCubicPath(configs: any, direction: 'v' | 'h', grid: Grid) {
  const { startPoint, endPoint } = configs;
  const allPoints = [startPoint, ...(configs.controlPoints?.concat?.() ?? []), endPoint];
  ...
  for (let i = 1; i < allPoints.length - 1; i++) {
    ...
    const length = Math.min(
      Math.sqrt(Math.pow(point2[0] - vertex[0], 2) + Math.pow(point2[1] - vertex[1], 2)),
      Math.sqrt(Math.pow(point1[0] - vertex[0], 2) + Math.pow(point1[1] - vertex[1], 2))
    );
    if (length <= 0) {
      path.push(['L', vertex[0], vertex[1]]);
      continue;
    }
    const [cp1, cp2, , r, angle] = getArcControlPoints(point1, vertex, point2, length, false);
    if (!r) {
      path.push(['L', cp2[0], cp2[1]]);
    } else {
      path.push(['L', cp1[0], cp1[1]]);
      path.push(['Q', vertex[0], vertex[1], cp2[0], cp2[1], (r as number) * (angle as number)]);
    }
  }
  path.push(['L', endPoint[0], endPoint[1]]);
  return path;
...
const stops = [
  0.125, 0.16666666666666666, 0.20833333333333331, 0.25, 0.29166666666666663, 0.3333333333333333, 0.375,
  0.41666666666666663, 0.4583333333333333, 0.5, 0.5416666666666666, 0.5833333333333333, 0.625, 0.6666666666666666,
...
function getCubicPath(configs: any, direction: 'v' | 'h') {
  ...
  const cp1 =
    direction === 'h'
      ? [(endPoint[0] - startPoint[0]) * curvePosition[0] + startPoint[0], startPoint[1]]
      : [startPoint[0], (endPoint[1] - startPoint[1]) * curvePosition[0] + startPoint[1]];
  const cp2 =
    direction === 'h'
      ? [(endPoint[0] - startPoint[0]) * curvePosition[1] + startPoint[0], endPoint[1]]
      : [endPoint[0], (endPoint[1] - startPoint[1]) * curvePosition[1] + startPoint[1]];
  const controlPoints = adjustControlPoints(configs, [cp1, cp2], direction);
  const path: (string | number)[][] = [
    ['M', startPoint[0], startPoint[1]],
    ['C', controlPoints[0][0], controlPoints[0][1], controlPoints[1][0], controlPoints[1][1], endPoint[0], endPoint[1]],
  ];
  return path;
}
