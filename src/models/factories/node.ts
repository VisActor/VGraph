...
import { ELEMENT_TYPES, NODE_THEMES, NODE_TYPES } from '../../consts/node_types';
...
function getTextAlignConfigs(configs: NodeConfigs, textConfigs: LabelConfigs) {
  const { offsetX = 0, offsetY = 0, textAlign = 'left', textBaseline = 'middle' } = textConfigs;
  let { paddingTop = 0, paddingRight = 0, paddingBottom = 0, paddingLeft = 0, padding } = configs;
    ...
    if (theme === NODE_THEMES.FILLED) {
      themeStyles = {
        ...
        icon: {
          ...
          background: {
            // 实心风格为3/4处
            fillStyle: background?.fillStyle || colorParser(color).lerp('#ffffff', 0.25).hex(),
          },
        },
        title: {
      ...
      themeStyles = {
        ...
        icon: {
          ...
          background: {
            // 线框风格为1/4处
            fillStyle: background?.fillStyle || colorParser(color).lerp('#ffffff', 0.75).hex(),
          },
        },
        title: {
      ...
      themeStyles = {
        ...
        icon: {
          ...
          background: {
            // 默认风格为1/8处
            fillStyle: background?.fillStyle || colorParser(color).lerp('#ffffff', 0.875).hex(),
          },
        },
      };
    }
...
export function registerMetaNodes() {
  ...
  register(
    ...
    NODE_TYPES.IMAGE,
    {
      ...
      getConfigsForShape(configs: NodeConfigs) {
        const { label, theme = NODE_THEMES.OUTLINED, color = '#7F8897' } = configs;
        if (configs.color) {
          configs.strokeStyle = theme === NODE_THEMES.OUTLINED ? color : undefined;
          configs.fillStyle = theme === NODE_THEMES.OUTLINED ? colorParser(color).lerp('#ffffff', 0.875).hex() : color;
        }
        if (!label) {
          return configs;
        }
        ...
        configs.label = {
          ...
          textAlign: 'center',
          textBaseline: 'top',
          y: configs.height! / 2 + (labelConfigs.offsetY ? labelConfigs.offsetY : 4),
          ...labelConfigs,
        };
        return configs;
      },
  ...
  register(
    ...
    NODE_TYPES.ICON,
    {
      ...
      getConfigsForShape(configs: NodeConfigs) {
        const { width, height, label, size, radius = 2, theme = NODE_THEMES.LIGHTED, color = '#7F8897' } = configs;
        ...
        icon = {
          ...
          fillStyle: icon.fillStyle ? icon.fillStyle : theme === NODE_THEMES.FILLED ? '#ffffff' : color,
          ...icon,
        };
        if (label) {
          let text: any = label;
  ...
  register(
    ...
    NODE_TYPES.CATEGORY,
    {
      ...
      getCategoryConfigs(layer: Layer, configs: NodeConfigs) {
        ...
        if (categoryConfigs.radius && categoryConfigs.radius[0] > CATEGORY_WIDTH) {
    ...
    NODE_TYPES.IMAGE_TAG,
    {
      ...
      getConfigsForShape(nodeData: any) {
        ...
        return {
          ...
          label: label
            ? {
              width: nodeData.width - DEFAULT_PADDING * 2 - img.size - DEFAULT_LABEL_PADDING,
