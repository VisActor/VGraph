import { Layer, Path, Quadratic, Cubic, Shape, PathConfigs } from '../../renderer';
import { normalizePadding, getNormalizedRad, normalizeVector } from '../../utils';
...
import { getArcControlPoints, getLoopDist, getDefaultLoopPath, getRoundLoopPath, getArcLoopPath } from './path';
...
export const edgeBase = Object.assign({}, Base, {
  ...
  getCustomLabelConfigs(shape: Shape, configs: EdgeConfigs) {
    ...
    const { offsetX = 0, offsetY = 0, autoRotate = false, position } = labelConfigs;
    ...
    if (autoRotate) {
      ...
      style.textAlign = percent === 0 ? 'start' : position === 1 ? 'end' : 'center';
    }
    return style;
  },
  ...
  register(
    ...
    EDGE_TYPES.HLINE,
    {
      getPath(configs: any) {
        ...
        if ((lineX < startPoint[0] && lineX < endPoint[0]) || (lineX > startPoint[0] && lineX > endPoint[0])) {
          console.warn('Invalid curve position, will fallback to default configs');
        ...
        if (Math.abs(dy) > 0) {
          ...
          if (
            ...
            r = Math.min(0.5 * Math.abs(dy), curvePosition * Math.abs(dx), (1 - curvePosition) * Math.abs(dx));
          } // 逻辑与tuningLine 统一，即 r 取能取到的最大值
          // if (startPoint[0] !== lineX - xTowards * r){ // 两点不同时才绘制直线
          //   path.push(['H', lineX - xTowards * r]);
          // }
          path.push(['H', lineX - xTowards * r]);
          path.push(['Q', lineX, startPoint[1], lineX, startPoint[1] + yTowards * r, 0.5 * Math.PI * r]);
          // if (startPoint + yTowards * r !== endPoint[1] - yTowards * r) { // 两点不同时才绘制直线
          //   path.push(['V', endPoint[1] - yTowards * r]);
          // }
          path.push(['V', endPoint[1] - yTowards * r]);
          path.push(['Q', lineX, endPoint[1], lineX + xTowards * r, endPoint[1], 0.5 * Math.PI * r]);
        }
  ...
  register(
    ...
    EDGE_TYPES.VLINE,
    {
      getPath(configs: any) {
        ...
        if ((lineY < startPoint[1] && lineY < endPoint[1]) || (lineY > startPoint[1] && lineY > endPoint[1])) {
          console.warn('Invalid curve position, will fallback to default configs');
        ...
        if (Math.abs(dx) > 0) {
          ...
          if (
            ...
            r = Math.min(0.5 * Math.abs(dx), curvePosition * Math.abs(dy), (1 - curvePosition) * Math.abs(dy));
          } // TODO: 由于 curvePosition 的存在， 所以应该分为 r1 和 r2
          // if (startPoint[1]!== lineY - yTowards * r){ // 两点不同时才绘制
          //   path.push(['V', lineY - yTowards * r]);
          // }
          path.push(['V', lineY - yTowards * r]);
          path.push(['Q', startPoint[0], lineY, startPoint[0] + xTowards * r, lineY, 0.5 * Math.PI * r]);
          path.push(['H', endPoint[0] - xTowards * r]);
          path.push(['Q', endPoint[0], lineY, endPoint[0], lineY + yTowards * r, 0.5 * Math.PI * r]);
        }
  ...
  register(
    ...
    EDGE_TYPES.TURNINGLINE,
    {
      getPath(configs: any) {
        ...
        const path: (string | number)[][] = [['M', startPoint[0], startPoint[1]]];
        if (!controlPoints) {
          controlPoints = getTurningControlPoint(source, target, curvePosition, startPoint, endPoint);
        }
        for (let i = 0; i < controlPoints.length; i++) {
          const point1 = i === 0 ? startPoint : controlPoints[i - 1];
          const vertex = controlPoints[i];
          const point2 = i === controlPoints.length - 1 ? endPoint : controlPoints[i + 1];
          ...
          const [cp1, cp2, , r, angle] = getArcControlPoints(point1, vertex, point2, length, isRadius);
          ...
          if (!r) {
            ...
            path.push(['Q', vertex[0], vertex[1], cp2[0], cp2[1], (angle as number) * (r as number)]);
          }
        }
        path.push(['L', endPoint[0], endPoint[1]]);
        return path;
    ...
    EDGE_TYPES.QUADRATIC,
    {
      ...
      getPoints(configs: any) {
        ...
        if (!controlPoints) {
          ...
          controlPoints = [getCurveControlPoint(startPoint, endPoint, curvePosition, curveOffset)];
        }
    ...
    EDGE_TYPES.CUBIC,
    {
      getPoints(configs: any) {
        ...
        if (!controlPoints || controlPoints.length !== 2) {
          controlPoints = [
            getCurveControlPoint(startPoint, endPoint, curvePosition[0], curveOffset[0]),
            getCurveControlPoint(startPoint, endPoint, curvePosition[1], curveOffset[1]),
          ];
  ...
  register(
    ...
    EDGE_TYPES.HCUBIC,
    {
      getPoints(configs: any) {
        ...
        const cp1 = [(endPoint[0] - startPoint[0]) * curvePosition[0] + startPoint[0], startPoint[1]];
        const cp2 = [(endPoint[0] - startPoint[0]) * curvePosition[1] + startPoint[0], endPoint[1]];
  ...
  register(
    ...
    EDGE_TYPES.VCUBIC,
    {
      getPoints(configs: any) {
        const { startPoint, endPoint, styles = {} } = configs;
        const { curvePosition = [0.5, 0.5] } = styles;
        const cp1 = [startPoint[0], (endPoint[1] - startPoint[1]) * curvePosition[0] + startPoint[1]];
        const cp2 = [endPoint[0], (endPoint[1] - startPoint[1]) * curvePosition[1] + startPoint[1]];
...
export function registerEdge(name: string, configs: any) {
  ...
  if (
    extend &&
    [EDGE_TYPES.QUADRATIC, EDGE_TYPES.CUBIC, EDGE_TYPES.LOOP, EDGE_TYPES.HCUBIC, EDGE_TYPES.VCUBIC].includes(extend)
  ) {
    base = edgeBase;
  }
...
export function getCurveControlPoint(startPoint: number[], endPoint: number[], t: number, offset: number) {
  const point = [(1 - t) * startPoint[0] + t * endPoint[0], (1 - t) * startPoint[1] + t * endPoint[1]];
  const tan = normalizeVector([endPoint[0] - startPoint[0], endPoint[1] - startPoint[1]]);
...
export function adjustControlPoints(configs: any, controlPoints: number[][], direction: 'v' | 'h' = 'v') {
  ...
  if (source.get('anchors') && isRectIntersect(edgeBox, sourceBBox)) {
    ...
    controlPoints[0] = getReversedCurveControlPoint(sourceCenter, targetCenter, startPoint, offset);
  }
  if (source.get('anchors') && isRectIntersect(edgeBox, targetBBox)) {
    const offset = Math.abs(curveOffset[1]) * 3;
    controlPoints[1] = getReversedCurveControlPoint(targetCenter, sourceCenter, endPoint, offset);
  }
...
function getReversedCurveControlPoint(sourcePoint: any, targetPoint: any, point: number[], offset: number) {
