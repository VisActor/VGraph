import { ANCHOR_TYPES, ELEMENT_TYPES, NODE_TYPES } from '../../consts/node_types';
...
export function getNodeMethods(name?: string | number) {
  return (
    (name && getShapeMethods(ELEMENT_TYPES.NODE, name + '')) || getShapeMethods(ELEMENT_TYPES.NODE, NODE_TYPES.RECT)
  );
}
...
export function getEdgeMethods(name?: string | number, configs?: any) {
  ...
  return (
    (name && getShapeMethods(ELEMENT_TYPES.EDGE, name + '')) || getShapeMethods(ELEMENT_TYPES.EDGE, EDGE_TYPES.LINE)
  );
}
...
export function getAnchorMethods(name?: string) {
  ...
  return getShapeMethods(ELEMENT_TYPES.ANCHOR, name) || getShapeMethods(ELEMENT_TYPES.ANCHOR, ANCHOR_TYPES.DOT);
}
